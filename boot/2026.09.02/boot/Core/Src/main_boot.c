...
static int backup_app_to_bak(void)
{
    ...
    uint32_t addr = APP_ADDRESS;
    uint32_t remaining = OTA_FW_PART_SIZE;
    uint32_t off = 0;

    for (uint32_t b = 0; b < OTA_FW_PART_SIZE; b += (64UL * 1024UL))
    {
...
#endif
        burn_spi_to_internal(OTA_FW_BAK_OFFSET, OTA_FW_PART_SIZE); /* 回滚 */
        meta->state = OTA_STATE_FAILED;
        write_meta(meta);
        return;
    }
...
#endif
        burn_spi_to_internal(OTA_FW_BAK_OFFSET, OTA_FW_PART_SIZE);
        meta->state = OTA_STATE_FAILED;
    }
    else
    {
