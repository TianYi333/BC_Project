...
- 8 路 = UART4/5/7/8 + USART1/2/3/6，115200 8N1；是否走 DMA 由具体 UART 决定：UART7/8 无 DMA、走中断，其余 6 路有 DMA。统一 `HAL_UARTEx_ReceiveToIdle_DMA/IT`，方向脚各路 `*_RTS_Pin`。
- **已确认的真实通道映射（CHx ↔ UART，板级接线为准，2026-09-11 核对）**：
  - CH1→UART7 / CH2→UART8 / CH3→USART3 / CH4→UART5 / CH5→USART1 / CH6→USART6 / CH7→UART4 / CH8→USART2
  - 即 `g_ports[0..7]` 对应 huart7/8/3/5/1/6/4/2（见 sensor_hub.c）。改接线只需同步改 `g_ports[]`。
- 从机地址 `0x01`，功能码 `04`（写 `06` 预留）。寄存器：`voltage_flag(0)`/`temp_flag(1)`/`temperature float32(2-3)`；**无电压寄存器** → `voltage` 恒 0。
- `SH_DEBUG_UART_ID`（默认 1，0=不占）可临时占一路作 printf 口；CH1=UART7 是中断接收那一路。
...
### ⑧ 上位机协议报文格式 & 自动生成工具（2026-09-11 定稿）
- 设备是 **TCP 客户端**，主动连上位机监听的 **50010**；UDP 发现/online 走 **50000 广播**。所有 JSON 报文带 `sign` = `HMAC-SHA256(KEY, 规范化原文)` 取前 4 字节大写 hex。密钥见 `UserCode/Logic/project_config.h`：`HMAC_KEY="0123456789abcdef0123456789abcdef"`。
- **⚠️ ts 字段类型不统一（最易错）**：`register`/`device_online`/`upgrade_start`/`sensor_data_ack`/`mode_set_req`/`config_set(UDP)` 里 `ts` 是**字符串**；`device_reboot` 和 `config_set_req` 里 `ts` 是**数字**。`config_set_req` 还做了**递归字典序排序**。规范化原文必须与 `net_comm_task.c` 的 snprintf / cJSON_SortObjectRecursive 逐字节一致，否则设备 `sign check failed` 直接丢包。
- 已做单文件网页生成器：**`tools/protocol_test_gen.html`**（纯 JS HMAC-SHA256，离线可用，已用 Node crypto 对拍全部 7 类签名一致）。覆盖 register/device_online/device_reboot/config_set_req/upgrade_start/sensor_data_ack/mode_set_req/config_set(UDP) 共 8 种，输出可直接贴网络助手。

## 待办 / 注意
- `sensorhub_write_mode()` 仅本地记录，未发 06 写帧（传感器暂无模式寄存器）。
- `SensorHub_Task` 栈 1024 字偏小（含 cJSON+HMAC+malloc），建议调到 2048~3072 字。
- RS485 回显测试：`SH_LOOPBACK_ECHO=1` 时每路(非调试口)收到数据原样发回；调试口那路作控制台不回显。测全部 8 路需换 `SH_DEBUG_UART_ID` 分次覆盖（无独立调试串口）。
