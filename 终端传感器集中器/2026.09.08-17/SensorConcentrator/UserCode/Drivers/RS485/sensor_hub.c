...
/* ============================ 端口映射表 ============================
 * sensor_id 1..8 与 UART、RS485 方向引脚（DE/RE）的对应关系。
 * 前 6 路用 DMA 接收，后 2 路（UART7/8）用中断接收。
 * 如硬件接线不同，仅需调整本表（use_dma 与 de_port/de_pin）。
 */
typedef struct {
...
static SensorPort_t g_ports[SENSOR_HUB_COUNT] = {
    /* sensor 1 */ { &huart4,  GPIOA, UART4_RTS_Pin,   1 },   /* UART4  DMA */
    /* sensor 2 */ { &huart5,  GPIOE, UART5_RTS_Pin,   1 },   /* UART5  DMA */
    /* sensor 3 */ { &huart1,  GPIOD, USART1_RTS_Pin,  1 },   /* USART1 DMA */
    /* sensor 4 */ { &huart2,  GPIOD, USART2_RTS_Pin,  1 },   /* USART2 DMA */
    /* sensor 5 */ { &huart3,  GPIOE, USART3_RTS_Pin,  1 },   /* USART3 DMA */
    /* sensor 6 */ { &huart6,  GPIOG, USART6_RTS_Pin,  1 },   /* USART6 DMA */
    /* sensor 7 */ { &huart7,  GPIOA, UART7_RTS_Pin,   0 },   /* UART7  中断 */
    /* sensor 8 */ { &huart8,  GPIOE, UART8_RTS_Pin,   0 },   /* UART8  中断 */
};
...
void SensorHub_Init(void)
{
    ...
    for (uint8_t i = 0; i < SENSOR_HUB_COUNT; i++)
    {
        ...
        UART_HandleTypeDef *huart = g_ports[i].huart;
        ...
        {
            /* 将 RX DMA 由 CubeMX 的 CIRCULAR 改为 NORMAL，
             * 配合 ReceiveToIdle 每次 IDLE 后重新启动，避免环形计数越界问题。 */
...
void SensorHub_Task(const void *argument)
{
    ...
    for (;;)
    {
        if (xQueueReceive(g_sensor_frame_queue, &f, portMAX_DELAY) == pdPASS)
        {
            ...
            printf("[S%u RX] ", (unsigned)f.sensor_id);
            ...
            {
#if (SH_DEBUG_UART_ID && SH_DEBUG_DUMP_FRAME)
                printf("  -> temp=%ld vflag=%u tflag=%u mode=%u\r\n",
                       (long)sd.temperature,
