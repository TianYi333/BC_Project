...
/* ============================ 类 Modbus 功能码 ============================ */
#define SH_FUNC_READ_HOLDING    0x03    /* 读保持寄存器（模式寄存器 0x0007 用） */
#define SH_FUNC_READ            0x04    /* 读输入寄存器 */
#define SH_FUNC_WRITE           0x06    /* 写单个保持寄存器 */
...
/* 工作模式寄存器（传感器协议寄存器表：0x0007，功能码 03/06，uint16，取值 0~1）
 *   0 = Modbus 问答模式；1 = 主动上报模式。写入值与 SENSOR_MODE_REG/ACTIVE 直传对应。 */
#define SH_REG_MODE             0x0007

/* 是否真正下发模式写帧（1=发送功能码 06 并校验回显应答；0=仅本地记录） */
#define SH_MODE_WRITE_ENABLE    1

/* 等待 FC06 写应答（回显帧）的超时，ms。超时判定传感器离线。 */
#define SH_MODE_RESP_TIMEOUT_MS 500
...
/* 写单个保持寄存器（功能码 06）：发送后阻塞等待从机回显应答并校验。
 * 返回 0 = 应答校验通过；-1 = 参数/资源错误；-2 = 该路被调试口占用；
 *      -3 = 应答超时（判定传感器离线）；-4 = 应答内容/CRC 校验失败。 */
