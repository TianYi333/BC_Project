...
/**
 * @brief 通过 Modbus（FC06 写保持寄存器 0x0007）切换终端传感器工作模式
 * @note  寄存器 0x0007：0 = Modbus 问答模式，1 = 主动上报模式（uint16，值直传）。
 *        SensorHub_WriteReg() 内部阻塞等待从机回显应答（最长 SH_MODE_RESP_TIMEOUT_MS，
 *        当前 500ms），mode_set_req 带 sensor_id=0 时最多 8 路串行执行。
 * @retval 0-成功, 2-写入失败, 3-传感器离线
 */
static int8_t sensorhub_write_mode(uint8_t sensor_id, uint8_t mode)
{
    if(sensor_id < 1 || sensor_id > 8) return MODE_SET_WRITE_FAIL; // 非法ID
    if(mode != SENSOR_MODE_REG && mode != SENSOR_MODE_ACTIVE) return MODE_SET_INVALID;

    /* FC06 写保持寄存器 SH_REG_MODE(0x0007)，应答回显校验通过才算成功 */
#if SH_MODE_WRITE_ENABLE
    int8_t ret = SensorHub_WriteReg(sensor_id, SH_REG_MODE, mode);
#else
    (void)mode;
    return 0;                        // 未使能下发：仅本地记录（旧行为）
#endif
    if(ret == 0)
    {
        return 0;                    // 应答校验通过
    ...
    if(ret == -3)
    {
        return MODE_SET_OFFLINE;     // 应答超时 -> 传感器离线
    }
    return MODE_SET_WRITE_FAIL;      // 发送失败 / 回显校验失败 / 该路被调试口占用
}
...
void tcp_send_sensor_data(const SensorData_t *sd)
{
    ...
    // 顶层字段严格字典序：device_id → hw_version → mode → model → sensor_id →
    // seq → sw_version → temp_flag → temperature → ts → type → voltage → voltage_flag
    // 注意：mode 必须排在 model 之前（"mode" 是 "model" 的前缀，短者优先），
    //       否则网关按字典序重排后的签名源串与本机不一致，sign 校验必失败。
    cJSON_AddStringToObject(root, "device_id", g_device_id);
