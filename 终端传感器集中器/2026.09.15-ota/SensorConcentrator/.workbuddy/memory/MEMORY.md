...
- **构建（两工程）**：App 本工程预设构建，须确认 `SystemInit` 写 `VTOR=0x080A0000`；Boot `D:\Desktop\SensorConcentrator_boot`：**统一只用 `build/`（Makefile 生成器）**，用户本机可正常构建（2026-09-12 11:44 重建成功）。⚠️ 沙箱 shell 的 PATH 里**没有 make**，`cmake --build build` 在本会话会失败；确需在此构建时只能临时 `cmake -B <tmpdir> -S . -DCMAKE_TOOLCHAIN_FILE=cmake/gcc-arm-none-eabi.cmake -G Ninja`（ninja @ `/c/Users/Lenovo/AppData/Local/stm32cube/bundles/ninja/1.13.2+st.1/bin`、gcc @ `/d/APP/GCC-ARM/bin`），**用完即删**，或干脆让用户自己跑 `cmake --build build`。曾建的 `build_ninja/` 已按用户要求删除。其 `Core/Inc/ota_common.h` 是本工程 `ota_common.h` 的**独立快照**，改偏移须两边同步。
- **Boot 调试打印（2026-09-12 新增）**：`Core/{Inc,Inc}/boot_uart.{h,c}`——寄存器级 UART7 **TX=PF7(AF8)** + RS485 方向脚 **DE=PA5**（与 App `SH_DEBUG_UART_ID=1` 的 CH1 同口），115200 8N1，波特率基准 **HSI 16MHz / PCLK1 不分频**（`BOOT_PCLK1_HZ`；若 boot 开 PLL 必须改）。自写 `boot_printf()` 不接 newlib（避 nano+gc-sections 的 `_write_r` 坑），`boot_dump_flash_sr()` 解码 FLASH->SR 错误位；`main_boot.c` 全流程 `[BOOT]` 打印（meta/CRC/逐扇区擦除/烧写进度/失败地址+SR/跳转前 SP-PC）。GPIO 成员名是 **`OTYPER`** 非 `OTYPE`。boot 固件 3768B→8008B。
...
- **⑨ 看门狗（2026-09-12）**：`iwdg.c` 现为 **Prescaler=128 / Reload=4095 / Window=4095 → 超时 16.4 s**（LSI 离散 17~47kHz，最坏约 11 s；**原 Prescaler=4 只有 0.512 s，是"开狗就无限重启"的元凶**）。⚠️ `MX_IWDG_Init()` 在 `main.c` 里早于 `osKernelStart()`，而喂狗靠 RTOS 任务 → **绝不能在 `MX_FREERTOS_Init()` 里 `osDelay()`**（调度器未启动时 `pxCurrentTCB` 已指向最高优先级任务即 `IWDGTask`，delay 会把喂狗任务自己挂起；原 `osDelay(3000)` 已删）。⚠️ **IWDG 软件模式（选项字节 `IWDG_SW=1`，出厂默认）下，任何系统复位（含 `NVIC_SystemReset`、IWDG 自身触发的复位）都会停止 IWDG**（ST 官方：*"Once enabled, it can only be disabled by a reset"*）→ 上电与 OTA 软复位进 boot 时狗都没在跑，**boot 不需要喂狗**（2026-09-12 由用户纠正，此前误记为"软复位停不掉"）。只有把选项字节配成硬件看门狗（`IWDG_SW=0`）才会上电自动运行，那时 `main_boot.c` 必须在擦除/烧写沿途喂狗。真正的喂狗盲区只剩 App 侧**关中断的 Flash 操作**：OTA 擦 `firmware_a` 3×128KB（单次 max 2 s，连续最长 6 s）、FlashDB GC。
