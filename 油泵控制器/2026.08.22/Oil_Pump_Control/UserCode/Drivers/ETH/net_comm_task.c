...
static void FillDeviceConfig(DeviceConfigParam_t *cfg)
{
    ...
    cfg->ref_pressure      = (int32_t)(sys_cfg.ref_pressure * 100.0f);
    cfg->max_pressure      = (int32_t)(sys_cfg.max_pressure * 100.0f);
    cfg->press_hysteresis  = (int32_t)(sys_cfg.press_hysteresis * 100.0f);
    cfg->overpress_margin  = (int32_t)(sys_cfg.overpress_margin * 100.0f);

    cfg->max_motor_speed   = (int32_t)(sys_cfg.max_motor_speed);
    cfg->min_motor_speed   = (int32_t)(sys_cfg.min_motor_speed);

    cfg->max_oil_temp      = (int32_t)(sys_cfg.max_oil_temp * 100.0f);
    cfg->min_liquid_level  = (int32_t)(sys_cfg.min_liquid_level * 100.0f);

    cfg->pid_kp            = (int32_t)(sys_cfg.pid_kp * 100.0f);
    cfg->pid_ki            = (int32_t)(sys_cfg.pid_ki * 100.0f);
    cfg->pid_kd            = (int32_t)(sys_cfg.pid_kd * 100.0f);
}
...
static int32_t ParseConfigSetReq(cJSON *dev_cfg_obj, DeviceConfigParam_t *tmp_cfg)
{
    ...
    */
    // 全部校验通过，更新全局sys_cfg
    sys_cfg.ref_pressure        = (float)tmp_cfg->ref_pressure / 100.0f;
    sys_cfg.max_pressure        = (float)tmp_cfg->max_pressure / 100.0f;
    sys_cfg.press_hysteresis    = (float)tmp_cfg->press_hysteresis / 100.0f;
    sys_cfg.overpress_margin    = (float)tmp_cfg->overpress_margin / 100.0f;

    sys_cfg.max_motor_speed     = tmp_cfg->max_motor_speed;
    sys_cfg.min_motor_speed     = tmp_cfg->min_motor_speed;

    sys_cfg.max_oil_temp        = (float)tmp_cfg->max_oil_temp / 100.0f;
    sys_cfg.min_liquid_level    = (float)tmp_cfg->min_liquid_level / 100.0f;

    sys_cfg.pid_kp              = (float)tmp_cfg->pid_kp / 100.0f;
    sys_cfg.pid_ki              = (float)tmp_cfg->pid_ki / 100.0f;
    sys_cfg.pid_kd              = (float)tmp_cfg->pid_kd / 100.0f;
