...
static void safe_tcp_close(void)
{
    ...
    do {
        ...
        // 强制销毁PCB，同步回收TCP段、发送缓冲区
        tcp_abort((struct tcp_pcb *)tcp_pcb);
        LOG("[TCP] tcp_abort force release all send buffer");
...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    else if(strcmp(type->valuestring, "remote_control") == 0)
    {
        ...
        valve_state_val    = (uint8_t)obj_valve_state->valuedouble;
        fault_reset_val    = (uint8_t)obj_fault_reset->valuedouble;

        /* HMAC签名源串，严格字典序，与网关完全对齐 */
        snprintf(tcp_sign_buf_2048, sizeof(tcp_sign_buf_2048),
            "{\"device_id\":\"%s\",\"fault_reset_req\":%u,\"seq\":%lu,\"sys_start\":%u,\"ts\":%s,\"type\":\"remote_control\",\"valve_state\":%u}",
            g_device_id,
            fault_reset_val,
            (unsigned long)curr_seq,
            sys_start_val,
    ...
    if(strcmp(type->valuestring, "remote_control") == 0)
    {
        ...
        if(SysStatus_ReadSnapshot(&snap_tmp))
        {
            uint16_t global_fault = (snap_tmp.sys_fault_bit != 0U) ? 1U : 0U;

            /* sys_start:1强制停机，5恢复自动，0无操作 */
            if(sys_start_val == 1U)
            {
            ...
            {
                LOG("remote ctrl: fault_bit exist, skip valve_state\r\n");
            }
            ...
            LOG("remote_control apply ok: sys_start=%u valve=%u fault_reset_req=%u\r\n",
