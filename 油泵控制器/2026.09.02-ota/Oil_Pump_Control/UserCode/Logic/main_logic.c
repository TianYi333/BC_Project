...
// 电机启停事件记录存储
MotorEventRecord g_motor_event_records[MOTOR_EVENT_MAX_CNT] = {0};
uint16_t g_motor_event_cnt = 0U;

// 电机启停事件记录互斥锁（多任务可能并发写：MainLogicTask/ModbusMasterTask/step_speed_ctrl_task）
osMutexId_t g_motor_event_mutex = NULL;

// 记录一条电机启停事件（指令态）。内部已加互斥锁，由 MainLogicTask 调用。
// event: MOTOR_EVT_START / MOTOR_EVT_STOP
void MotorEvent_Record(uint8_t event)
{
    // 时间获取（可能耗时，放在互斥锁外，缩短临界区）
    refresh_rtc_time();
    uint8_t hh = RtcTime.Hours;
    uint8_t mm = RtcTime.Minutes;
    uint8_t ss = RtcTime.Seconds;
    uint64_t sec = Get_Unix_Second();    // 秒级时间戳（UTC）：结构体 tick 与上报 tick_ts 同源

    if(g_motor_event_mutex == NULL)
    {
        // 互斥锁未创建（异常），放弃记录避免访问未初始化资源
        return;
    }
    if(osMutexAcquire(g_motor_event_mutex, osWaitForever) != osOK)
    {
        return;
    }

    uint16_t evt_idx = g_motor_event_cnt;
    if(evt_idx >= MOTOR_EVENT_MAX_CNT)
    {
        memmove(&g_motor_event_records[0], &g_motor_event_records[1],
                sizeof(MotorEventRecord) * (MOTOR_EVENT_MAX_CNT - 1));
        evt_idx = MOTOR_EVENT_MAX_CNT - 1;
        g_motor_event_cnt = MOTOR_EVENT_MAX_CNT;
    }
    else
    {
        g_motor_event_cnt++;
    ...
    MotorEventRecord *rec = &g_motor_event_records[evt_idx];
    rec->event = event;
    rec->tick  = sec;   // 秒级时间戳（与上报字段 ts 同精度）

    osMutexRelease(g_motor_event_mutex);

    // 主动上报电机启停事件（TCP），在锁外调用，避免阻塞记录临界区
    tcp_send_motor_event_report(event, sec);

    LOG_Debug("Motor %s @ %02d:%02d:%02d",
              (event == MOTOR_EVT_START) ? "START" : "STOP",
              hh, mm, ss);
}
...
void SysStatus_Init(void)
{
    ...
    // 创建电机启停事件记录互斥锁（多任务并发写 g_motor_event_records 保护）
    if(g_motor_event_mutex == NULL)
    {
        g_motor_event_mutex = osMutexNew(NULL);
    }
}
...
int Cfg_SetDefault(SYS_CONFIG_T *cfg)
{
    ...
    cfg->ref_pressure = 15.0f;
    cfg->max_pressure = 30.0f;
    cfg->press_hysteresis = 0.3f;
    cfg->overpress_margin = 10.0f;

    cfg->max_motor_speed = 600.0f;
    cfg->min_motor_speed = 100.0f;

    cfg->max_oil_temp = 85.0f;
    cfg->min_liquid_level = 20.0f;
...
uint8_t SysStatus_ReadSnapshot(_SYS_STATUS *dst)
{
    ...
    if(sys_status_mutex != NULL)
    {
        ...
        {
            // 获取锁超时：降级裸拷贝，记录警告日志
            LOG_Debug("sys mutex acquire timeout, fallback raw read");
        }
    }
    else
    {
        // 互斥锁未创建，降级裸拷贝
        LOG_Debug("sys mutex NULL, fallback raw read");
    }
...
uint8_t SysStatus_WriteSnapshot(_SYS_STATUS *src)
{
    ...
    if(sys_status_mutex != NULL)
    {
        ...
        {
            LOG_Debug("sys mutex acquire timeout, write abort");
    ...
    {
        LOG_Debug("sys mutex NULL, write abort");
        // 锁不存在，禁止裸写全局状态，防止多任务并发篡改
        return 0;
    }
}

// =========全局配置 sys_cfg 加锁读写接口（独立 config_mutex）=========
uint8_t SysConfig_ReadSnapshot(SYS_CONFIG_T *dst)
{
    if(dst == NULL)
        return 0;
    ...
    {
        if(osMutexAcquire(config_mutex, pdMS_TO_TICKS(20U)) == osOK)
        {
            *dst = sys_cfg;
        ...
        {
            LOG_Debug("cfg mutex acquire timeout, fallback raw read");
        }
    }
    else
    {
        LOG_Debug("cfg mutex NULL, fallback raw read");
    }
...
uint8_t SysConfig_WriteSnapshot(SYS_CONFIG_T *src)
{
    if(src == NULL)
        return 0;
    ...
    if(config_mutex != NULL)
    {
        ...
        {
            LOG_Debug("cfg mutex acquire timeout, write abort");
            // 写入操作风险极高！锁拿不到，禁止裸写，直接放弃本次更新
            return 0;
        }
    }
    ...
    {
        LOG_Debug("cfg mutex NULL, write abort");
        // 锁不存在，禁止裸写全局配置，防止多任务并发篡改
        return 0;
    }
}

void SysConfig_Lock(void)
{
    if(config_mutex != NULL)
    {
        (void)osMutexAcquire(config_mutex, osWaitForever);
    }
}

void SysConfig_Unlock(void)
{
    if(config_mutex != NULL)
    {
        osMutexRelease(config_mutex);
...
void MainLogicTask(void *argument)
{
    ...
    static uint8_t last_motor_state     = MOTOR_STATE_WAIT;// 上一拍电机状态，用于检测启停跳变
    ...
    for(;;)
    {
        ...
        // 加锁读取配置，防止并发写入撕裂（多字段一致性）
        SYS_CONFIG_T cfg_tmp;
        SysConfig_ReadSnapshot(&cfg_tmp);
        float ref_p                 = cfg_tmp.ref_pressure;
        float max_pressure          = cfg_tmp.max_pressure;//最大压力阈值
        float overpress_margin      = cfg_tmp.overpress_margin;//压力超压阈值（%）
        float severe_overpress_thr  = max_pressure * (1.0f + overpress_margin / 100.0f);
        float max_oil_temp          = cfg_tmp.max_oil_temp;
        float min_liquid_level      = cfg_tmp.min_liquid_level;
        float PRESS_HYSTERESIS      = cfg_tmp.press_hysteresis;

        uint8_t motor_fault           = 0;//电机故障标志
        static uint8_t oil_temp_fault        = 0;//油温过高锁死故障标志（持久锁存，手动复位才清）
        static uint8_t low_liquid_fault      = 0;//油液过低锁死故障标志（持久锁存，手动复位才清）
        ...
        //   滞回清除：触发后须压力回落至目标压力(ref_p)以下才解除超压保护，
        //   避免压力在 severe_overpress_thr 附近抖动导致故障位反复置位/清除
        ...
        else if(curr_p <= ref_p)
        {
            // 压力已回落至目标压力，解除超压保护并重置防抖计数
            overpress_cnt = 0;
            severe_overpress_flag = 0;
        }
        // 滞回区(ref_p < curr_p < severe_overpress_thr)：severe_overpress_flag 保持不变

        if(severe_overpress_flag != 0U)
        {
            local_fault_bit |= SYS_ERR_SEVERE_OVERPRESS;
        }

        //3.油液过温保护（锁死故障，不可自动恢复，需手动复位才清）
        if(sys_tmp.adc_oil_temp >= max_oil_temp)
        {
            if(oil_temp_over_cnt >= FAULT_DEBOUNCE_CNT)
            {
                oil_temp_fault = 1;// 锁死故障：置位后持久，不随油温瞬时恢复而清零
            }
            else
            {
                oil_temp_over_cnt++;
        ...
        {
            oil_temp_over_cnt = 0;// 防抖计数复位；锁死故障标志 oil_temp_fault 不自动清零
        }
        if(oil_temp_fault != 0U)// 锁死故障：一旦触发，持续锁定直到手动复位
        {
            local_fault_bit |= SYS_ERR_OIL_TEMP_OVER;
        }

        //4.低液位保护（锁死故障，需手动复位才清）液位传感器异常时，不做液位判定，直接跳过
        if(g_fault_report.flg_liquid_sensor_err == 0U)
        {
            if(sys_tmp.liquid_level_pct < min_liquid_level)
            {
                ...
                {
                    low_liquid_fault = 1;// 锁死故障：置位后持久，不随液位瞬时恢复而清零
                }
                else
                {
                    low_liquid_cnt++;
            ...
            {
                low_liquid_cnt = 0;// 防抖计数复位；锁死故障标志 low_liquid_fault 不自动清零
            }
        }
        else
        {
            //传感器故障，防抖计数器清零，防止恢复后瞬间触发故障；已锁定的故障保持
            low_liquid_cnt = 0;
        }
        if(low_liquid_fault != 0U)// 锁死故障：一旦触发，持续锁定直到手动复位
        {
        ...
        if(fault_change_trig)
        {
            ...
            // 直接读取RTC硬件时间：RTC硬件按北京时间(UTC+8)存储，
            // refresh_rtc_time()填充的RtcTime即为北京时间，无需再做时区换算
            refresh_rtc_time();
            new_rec->hour = RtcTime.Hours;
            new_rec->min  = RtcTime.Minutes;
            new_rec->sec  = RtcTime.Seconds;

            // 时间戳使用标准unix秒(UTC)，与 MotorEventRecord.tick 同源，便于日志/协议对接
            new_rec->tick = Get_Unix_Second();
        }
        ...
        if(SysStatus_ReadSnapshot(&write_tmp))
        {
            ...
            if(fault_bit_changed || sensor_stat_changed)
            {
                ...
                if(write_ret == 0)
                {
                    LOG_Debug("Fault/sensor_status write mutex timeout");
                }
            }
        }
        ...
        if(auto_fault_exist || lock_fault_exist)
        {
            ...
            if(sys_tmp.sys_start != SYS_STATE_SETTING && sys_tmp.sys_start != SYS_STATE_FAULT)
            {
                ...
                if(SysStatus_ReadSnapshot(&write_tmp))
                {
                    ...
                    {
                        LOG_Debug("FAULT trig bit=0x%02X auto=%d lock=%d",
                                  local_fault_bit, auto_fault_exist, lock_fault_exist);
        ...
        if(lock_setting_mode)
        {
            ...
            switch(sys_tmp.sys_start)
            {
                case SYS_STATE_SETTING:
                {
                    ...
                    {
                        LOG_Debug("Enter SETTING Mode (Locked, auto-switch disabled)");
                        _SYS_STATUS write_tmp;
                        if(SysStatus_ReadSnapshot(&write_tmp))
                        {
        ...
        else
        {
            ...
            switch(sys_tmp.sys_start)
            {
                ...
                case SYS_STATE_FAULT:
                {
                    ...
                    {
                        LOG_Debug("System Enter FAULT State");
                    ...
                    if(lock_fault_exist == 0)// 无锁故障
                    {
                        ...
                        {
                            LOG_Debug("All auto-clear faults recovered, exit FAULT");
                            {
                                _SYS_STATUS write_tmp;
                                if(SysStatus_ReadSnapshot(&write_tmp))
                                {
                    ...
                    if(fault_reset_trig == 1U)// 故障复位上升沿触发
                    {
                        ...
                        {
                            LOG_Debug("Manual reset success, exit FAULT");
                            if(SysStatus_ReadSnapshot(&write_tmp))
                            {
                                uint8_t need_update = 0;
                                if(write_tmp.sys_start != SYS_STATE_STOP)
                                ...
                                {
                                    write_tmp.sys_fault_bit = 0U;
                                    low_liquid_fault = 0U;// 手动复位成功：清除锁死故障锁存
                                    oil_temp_fault   = 0U;
                                    need_update = 1;
                                }
                                if(need_update)
                        ...
                        {
                            LOG_Debug("Reset reject: Lock fault still exists");
                ...
                default:
                {
                    LOG_Debug("Unknown system state, force STOP");
                    _SYS_STATUS write_tmp;
                    if(SysStatus_ReadSnapshot(&write_tmp))
                    {
                        uint8_t need_update = 0;
        ...
        // ============ 电机启停事件记录（指令态） ============
        // 检测 motor_status.state 跳变：进入 RUN = 启动，离开 RUN = 停止。
        // RUN<->WAIT 不视为启停（WAIT 仅是软件等待态，电机未实际停转）。
        uint8_t cur_motor_state = sys_tmp.motor_status.state;
        if(cur_motor_state == MOTOR_STATE_RUN && last_motor_state != MOTOR_STATE_RUN)
        {
            MotorEvent_Record(MOTOR_EVT_START);
        }
        else if(cur_motor_state != MOTOR_STATE_RUN && last_motor_state == MOTOR_STATE_RUN)
        {
            MotorEvent_Record(MOTOR_EVT_STOP);
        }
        last_motor_state = cur_motor_state;

        /* OTA 启动确认（防砖确认点）：当 boot 烧录完成并置 DONE 后，
           首轮控制循环跑完到这里，把 ota_meta 改写为 CONFIRMED，
           阻断 boot 误判"未启动"而回滚到旧固件。
           函数内部静态标志保证每上电只生效一次。 */
        ota_app_confirm();
