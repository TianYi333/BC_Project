...
void sync_to_modbus() {
  ...
  usRegHoldingBuf[regID_IsRunning] = main_sys_status_1.sys_start;
...
void sync_from_modbus(_MB_REG mbMsg) {
  ...
  if (mbMsg.startAddr <= regID_IsRunning &&
    ...
    main_sys_status_1.sys_start = usRegHoldingBuf[regID_IsRunning];
  }
  ...
  if (regID_Injector <= mbMsg.startAddr &&
      (300 + (8 * 20) - 1) >= (mbMsg.startAddr + mbMsg.len)) {
    uint8_t tag = 0;
