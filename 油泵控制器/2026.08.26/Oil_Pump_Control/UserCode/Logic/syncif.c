...
void sync_from_sysdb() {
  ...
  SYS_CONFIG_T tmp = {0};
  size_t got = fdb_kv_get_blob(&kvdb, "sys_cfg", fdb_blob_make(&blob, &tmp, sizeof(SYS_CONFIG_T)));
  if(got == sizeof(SYS_CONFIG_T)) {
...
void sync_to_sysdb() {
  struct fdb_blob blob;
  SYS_CONFIG_T tmp;
  SysConfig_ReadSnapshot(&tmp);
  fdb_kv_set_blob(&kvdb, "sys_cfg", fdb_blob_make(&blob, &tmp, sizeof(SYS_CONFIG_T)));
}
