...
int Cfg_SetDefault(SYS_CONFIG_T *cfg)
{
    ...
    cfg->ref_pressure = 5.0f;
    cfg->max_pressure = 20.0f;
    cfg->press_hysteresis = 0.3f;
    cfg->overpress_margin = 10.0f;

    cfg->max_motor_speed = 2000.0f;
    ...
    cfg->press_cal_4ma_raw  = 12710.0f;
    cfg->press_cal_20ma_raw = 63549.0f;

    cfg->temp_cal_4ma_raw   = 12710.0f;
    cfg->temp_cal_20ma_raw  = 63549.0f;
...
uint8_t SysConfig_ReadSnapshot(SYS_CONFIG_T *dst)
{
    ...
    if(config_mutex != NULL)
    {
        ...
        {
            LOG_logic("cfg mutex acquire timeout, fallback raw read");
        }
    }
    else
    {
        LOG_logic("cfg mutex NULL, fallback raw read");
    }
...
uint8_t SysConfig_WriteSnapshot(SYS_CONFIG_T *src)
{
    ...
    if(config_mutex != NULL)
    {
        ...
        {
            LOG_logic("cfg mutex acquire timeout, write abort");
            // 写入操作风险极高！锁拿不到，禁止裸写，直接放弃本次更新
            return 0;
        }
    }
    ...
    {
        LOG_logic("cfg mutex NULL, write abort");
