...
static void safe_tcp_close(void)
{
    ...
    do {
        ...
        {
            tcp_abort(pcb);
            LOG("[TCP] tcp_close fail %d, fallback tcp_abort", (int)close_ret);
        }
        else
        {
            LOG("[TCP] tcp_close graceful close");
        }
    ...
    LOG("[TCP] clear %lu pending send msg", drop_count);
...
static void cJSON_SortObjectRecursive(cJSON *obj)
{
    ...
    qsort(item_arr, item_cnt, sizeof(cJSON*), key_cmp);
...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    if(strcmp(type->valuestring, "state_request") == 0)
    {
        ...
        {
            LOG("state_request query_type invalid!\r\n");
            goto exit;
        }
    ...
    else if(strcmp(type->valuestring, "remote_control") == 0)
    {
        ...
        {
            LOG("remote_control param invalid\r\n");
            goto exit;
        }

        uint8_t sys_start_val      = (uint8_t)obj_sys_start->valuedouble;
        int8_t  valve_state_val    = (int8_t)obj_valve_state->valuedouble;   // 支持‑1
        uint8_t fault_reset_val    = (uint8_t)obj_fault_reset->valuedouble;
    ...
    if(strcmp(type->valuestring, "remote_control") == 0)
    {
        ...
        if(SysStatus_ReadSnapshot(&snap_tmp))
        {
            /* sys_start:1切设置模式，5切停止模式，0无操作 */
            if(sys_start_val == 1U)
            {
                snap_tmp.sys_start = SYS_STATE_SETTING;
            }
            else if(sys_start_val == 5U)
            {
                snap_tmp.sys_start = SYS_STATE_STOP;
            }
            /* sys_start_val == 0：不修改snap_tmp.sys_start */

            /*
            * 允许阀门操作布尔条件：
            * Case1：已处于设置模式 并且 本条sys_start_val !=5
            * Case2：不在设置模式 并且 本条sys_start_val ==1
            */
            uint8_t permit_valve = 0U;
            if( (snap_tmp.sys_start == SYS_STATE_SETTING) && (sys_start_val != 5U) )
            {
                permit_valve = 1U;
            }
            else if( (snap_tmp.sys_start != SYS_STATE_SETTING) && (sys_start_val == 1U) )
            {
                permit_valve = 1U;
            }
            ...
            if(permit_valve == 1U)
            {
                if(valve_state_val == 0 || valve_state_val == 1)
                {
                    ...
                    LOG("remote ctrl: allow apply valve_state=%d\r\n", valve_state_val);
                }
                else
                {
                    LOG("remote ctrl: valve_state=%d skip(-1 no-op)\r\n", valve_state_val);
                }
            }
            else
            {
                LOG("remote ctrl: condition not meet, skip valve_state\r\n");
            }
            ...
            LOG("remote_control apply ok: sys_start=%u valve=%d fault_reset_req=%u\r\n",
                    sys_start_val, valve_state_val, fault_reset_val);
        }
        else
        {
            LOG("remote_control: read snapshot fail, discard cmd\r\n");
        }
    }
...
void tcp_send_motor_event_report(uint8_t event, uint64_t tick_ts)
{
    ...
    if(send_buf)
    {
        ...
        if(msg.data) {
            ...
            if(xQueueSend(tcp_send_queue, &msg, 0) != pdPASS) {
                vPortFree(msg.data);          // 队列满，丢弃
                LOG("TCP send queue full, drop motor_event_ack");
            }
        }
        cJSON_PortFree(send_buf);
        g_tcp_seq++;
...
void tcp_sender_task(void *arg)
{
    ...
    while(1)
    {
        ...
        if(pending > QUEUE_ALERT_THRESHOLD)
        {
            ...
            LOG("Queue overflow pre clear %lu pkt\r\n", clear);
        }
        ...
        while((free_buf < send_len) && (drop_cnt < MAX_DROP_CNT))
        {
            ...
            if(xQueueReceive(tcp_send_queue, &drop_msg, 0) == pdPASS)
            {
                ...
                {
                    vPortFree(drop_msg.data);
                    LOG("TCP buf full, drop pending pkt\r\n");
                }
                drop_cnt++;
            }
            else
        ...
        {
            LOG("TCP send buf full (need %d, free %d), drop pkt\r\n", send_len, free_buf);
            vPortFree(msg.data);
            osMutexRelease(tcp_send_mutex);
            UNLOCK_TCPIP_CORE();
            continue;
