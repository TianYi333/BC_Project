/**
 * @file oled_page.h
 * @brief OLED页面管理头文件
 * @author 梁伟
 * @date 2026-04-20
 */

#ifndef OLED_PAGE_H
#define OLED_PAGE_H

#include "u8g2.h"
#include "FreeRTOS.h"
#include "task.h"
#include "mbif.h"
// -------------------------- OLED 配置参数 --------------------------
#define OLED_WIDTH 256  // 屏幕宽度
#define OLED_HEIGHT 128 // 屏幕高度

// -------------------------- 开机画面配置 --------------------------
#define DEVICE_LOGO "BORCH Oil Pump" // 设备名称

// -------------------------- 菜单配置 --------------------------
#define MENU_TITLE "Main Menu"                          // 菜单标题
#define MENU_STATUS "Turn to navigate, enter to select" // 菜单状态提示
#define MENU_ITEM_H 20                                  // 菜单项高度
#define MENU_VISIBLE_MAX 4                              // 最大可见菜单项数量


// 菜单事件类型
typedef enum
{
    MENU_EVENT_UP,    // 向上导航
    MENU_EVENT_DOWN,  // 向下导航
    MENU_EVENT_ENTER, // 确认选择
    MENU_EVENT_BACK   // 返回上一级
} MenuEventType;

// 菜单项结构体
typedef struct
{
    char *item_name;        // 菜单项名称
    void (*callback)(void); // 菜单项回调函数
} MenuItem;

// 菜单结构体
typedef struct
{
    MenuItem *items;    // 菜单项数组
    uint8_t item_total; // 菜单项总数
    uint8_t cur_index;  // 当前选中项索引
    uint8_t top_index;  // 可见区域第一个项索引
} Menu;

// 全局菜单实例
extern Menu g_menu;
extern Menu system_settings_menu;
extern TickType_t g_menu_last_op_tick;

/**
 * @brief 显示开机画面
 * @param u8g2
 */
void show_oled_boot_page(u8g2_t *u8g2);

/**
 * @brief 显示菜单页面
 * @param u8g2
 */
void show_oled_main_menu_page(u8g2_t *u8g2);
/**
 * @brief 显示系统设置菜单
 * @param u8g2
 */
void show_oled_system_settings_menu_page(u8g2_t *u8g2);
/**
 * @brief 显示恢复出厂设置页面
 * @param u8g2
 * @param progress 恢复进度，范围0-1
 */
void show_restore_factory_settings_page(u8g2_t *u8g2 , float progress);
/**
 *  @brief 系统设置次级菜单事件处理函数
 */

void system_settings_menu_handle_event(MenuEventType event, uint8_t *menu);

/**
 * @brief 单元测试运行页面
 */
void unit_test_running_page(u8g2_t *u8g2, uint16_t p_angle, uint16_t q_angle, char sen_top, char sen_bottom, uint16_t running_tag);

/**
 * @brief 处理菜单事件
 * @param event 事件类型
 */
void menu_handle_event(MenuEventType event);

/**
 * @brief 初始化菜单
 * @param menu 菜单指针
 * @param items 菜单项数组
 * @param item_total 菜单项总数
 */
void menu_init(Menu *menu, MenuItem *items, uint8_t item_total);

/**
 * @brief 显示页面回调函数
 */
void p_model_init_callback(void);
void q_model_init_callback(void);
void test_running_callback(void);
void home_page_callback(void);
void reboot_callback(void);
void system_settings_callback(void);
void about_device_callback(void);
void menu_item_unit_test(void);


/**
 * @brief 系统设置菜单回调函数 
 */
void serial_port_settings_callback(void);
void p_model_speed_settings_callback(void);
void q_model_speed_settings_callback(void);
void restore_factory_settings_callback(void);


// 定义菜单项
extern MenuItem g_menu_items[7];

#define MENU_IDLE_TIMEOUT_MS 10000UL  // 20秒无操作超时返回首页
extern TickType_t g_menu_last_op_tick;
static inline void menu_reset_idle_timer(void)
{
    g_menu_last_op_tick = xTaskGetTickCount();
}

static inline uint8_t menu_is_idle_timeout(void)
{
    TickType_t now = xTaskGetTickCount();
    TickType_t diff = now - g_menu_last_op_tick;
    if (diff >= pdMS_TO_TICKS(MENU_IDLE_TIMEOUT_MS))
    {
        return 1;
    }
    return 0;
}
#endif /* OLED_PAGE_H */