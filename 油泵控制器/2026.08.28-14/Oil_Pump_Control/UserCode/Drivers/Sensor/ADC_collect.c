...
void ADCTask(void *argument)
{
    ...
    // LOG_ADC("ADC3_CR   = 0x%08lx", ADC3->CR);
    // LOG_ADC("ADC3_CFGR = 0x%08lx", ADC3->CFGR);
    ...
    for (;;)
    {
        if(adc_data_ready == 1)
        {
            ...
            float press_k = (20.0f - 4.0f) / (PRESS_CAL_20MA_RAW - PRESS_CAL_4MA_RAW);
            float press_off = 4.0f - press_k * PRESS_CAL_4MA_RAW;
            ...
            // 两点校准修正温度电流
            SysConfig_ReadSnapshot(&cfg_tmp);
            float temp_k = (20.0f - 4.0f) / (TEMP_CAL_20MA_RAW - TEMP_CAL_4MA_RAW);
            float temp_off = 4.0f - temp_k * TEMP_CAL_4MA_RAW;
