...
void PrintTaskStackAndCount(void)
{
    ...
    LOG("10,Task_InitTask             min stack left: %u bytes", uxTaskGetStackHighWaterMark(Task_InitTaskHandle));
    ...
    LOG("16,ADCTask                   min stack left: %u bytes", uxTaskGetStackHighWaterMark(ADCTaskHandle));
...
void Task_InitTask(void *argument)
{
  ...
  //injector_taskHandle = osThreadNew(injector_task, NULL, &injector_task_attributes);
