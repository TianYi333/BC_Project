	...
	if ((usAddress >= REG_Coils_START) && (usAddress + usNCoils <= REG_Coils_START + REG_Coils_NREGS)) {
		...
		switch (eMode) {
			...
			case MB_REG_WRITE:
				taskENTER_CRITICAL();
				while (iNCoils > 0) {
					xMBUtilSetBits(usRegCoilsBuf, iRegIndex, (unsigned char) (iNCoils > 8 ? 8 : iNCoils),
							*pucRegBuffer++);
					iRegIndex += 8;
					iNCoils -= 8;
				}
		...
		switch (eMode) {
			...
			case MB_REG_WRITE:
				...
				while (iNRegs > 0) {
					...
					usRegHoldingBuf[iRegIndex] = (usRegHoldingBuf[iRegIndex] & 0xFF00) | *pucRegBuffer++;
					iRegIndex++;
					iNRegs--;
				}
				taskEXIT_CRITICAL();
				xQueueSend(QUEUE_CONN, &mb_Msg, 500);
				break;
		}
	} else {
		eStatus = MB_ENOREG;
