...
const osMutexAttr_t config_mutex_attr = {
    .name = "config_mutex",
    .attr_bits = osMutexRecursive,        // 配置锁，保护全局 sys_cfg 读写，优先级继承
};
...
void MX_FREERTOS_Init(void) {
  ...
  config_mutex = osMutexNew(&config_mutex_attr);
  MsgQueue_Init();
  /* add threads, ... */
...
void PrintTaskStackAndCount(void)
{
    ...
    LOG_RTS("7 ,MBif_task                 min stack left: %lu bytes", uxTaskGetStackHighWaterMark(MBif_task_handler));
    LOG_RTS("8 ,MainLogicTask             min stack left: %lu bytes", uxTaskGetStackHighWaterMark(MainLogicTaskHandle));
    LOG_RTS("9 ,Task_InitTask             min stack left: %lu bytes", uxTaskGetStackHighWaterMark(Task_InitTaskHandle));
    LOG_RTS("10,Udp_discover_task         min stack left: %lu bytes", uxTaskGetStackHighWaterMark(Udp_discover_taskHandle));
    LOG_RTS("11,Tcp_client_task           min stack left: %lu bytes", uxTaskGetStackHighWaterMark(Tcp_client_taskHandle));
    LOG_RTS("14,TimeSyncTask              min stack left: %lu bytes", uxTaskGetStackHighWaterMark(TimeSyncTaskHandle));
    LOG_RTS("15,ADCTask                   min stack left: %lu bytes", uxTaskGetStackHighWaterMark(ADCTaskHandle));
    LOG_RTS("16,PressureControlTask       min stack left: %lu bytes", uxTaskGetStackHighWaterMark(PressureControlTaskHandle));
    LOG_RTS("17,ModbusMasterTask          min stack left: %lu bytes", uxTaskGetStackHighWaterMark(ModbusMasterTaskHandle));
