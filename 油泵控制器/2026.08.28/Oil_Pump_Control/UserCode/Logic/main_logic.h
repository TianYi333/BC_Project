...
// ===================== sys_cfg FlashDB 存储格式（magic + 版本 + CRC32 校验）=====================
#define SYS_CFG_MAGIC    0x4C554232U   // "LUB2"：标识带校验头的新格式
#define SYS_CFG_VERSION  1U            // 结构版本号，SYS_CONFIG_T 布局变动时 +1

// sys_cfg 存储记录头：位于 SYS_CONFIG_T 载荷之前
typedef struct
{
    uint32_t magic;    // 固定魔数，识别新格式
    uint32_t version;  // 结构版本号
    uint32_t crc32;    // 对紧随其后的 SYS_CONFIG_T 载荷的 CRC32 校验值
} SYS_CFG_HDR_T;

// 写入 FlashDB 的完整记录 = 记录头 + 配置载荷
typedef struct
{
    SYS_CFG_HDR_T hdr;
    SYS_CONFIG_T  cfg;
...
typedef struct
{
    ...
    uint8_t hour;            // 故障发生时 时（北京时间显示）
    uint8_t min;             // 分
    uint8_t sec;             // 秒
    uint64_t tick;           // RTC unix秒时间戳
} AlarmRecord;
...
// ============ 电机启停事件记录 ============
// 电机启停事件最大存储条数
#define MOTOR_EVENT_MAX_CNT 32U

// 电机启停事件类型
typedef enum
{
    MOTOR_EVT_STOP  = 0U, // 电机停止（离开 RUN 态）
    MOTOR_EVT_START = 1U  // 电机启动（进入 RUN 态）
} MOTOR_EVENT_E;

// 单条电机启停事件记录（由 MainLogicTask 监测 motor_status.state 进入/离开 RUN 判定，即指令态）
typedef struct
{
    uint8_t  event;      // 事件类型 MOTOR_EVT_START / MOTOR_EVT_STOP
    uint64_t tick;       // Unix秒级时间戳（UTC），与 Get_Unix_Second() 一致
} MotorEventRecord;

// 全局电机启停事件存储
extern MotorEventRecord g_motor_event_records[MOTOR_EVENT_MAX_CNT];
extern uint16_t g_motor_event_cnt;

// 记录一条电机启停事件（event: MOTOR_EVT_START / MOTOR_EVT_STOP）
// 内部已做互斥锁保护，由 MainLogicTask 调用
extern void MotorEvent_Record(uint8_t event);
...
extern osMutexId_t config_mutex;
...
// 设计：sys_cfg 是被 PressureControlTask(最高优先级,每周期读增益) / OLED菜单 /
//      Modbus从站 / 网络JSON 多任务并发读写的共享结构体。整结构体拷贝
//      (SYS_CONFIG_T x = sys_cfg) 在并发写入时会被撕裂，故统一加锁。
// 与 SysStatus 接口对称：Read/Write 为整结构体快照；Lock/Unlock 供多字段
// 临界区(如网络/Modbus 一次性写多个字段)使用，避免反复整结构体拷贝。
extern osMutexId_t config_mutex;
