...
static uint8_t reboot_load_persistent_info(char *reason_buf, uint16_t buf_len);
...
static void reboot_save_persistent_info(const char *reason)
{
    ...
    fdb_kv_set_blob(&net_kvdb, KV_KEY_REBOOT_INFO, blob);
...
static uint8_t reboot_load_persistent_info(char *reason_buf, uint16_t buf_len)
{
    ...
    blob->buf = (uint8_t *)&info;
    blob->size = sizeof(RebootPersistentInfo_t);
    size_t read_len = fdb_kv_get_blob(&net_kvdb, KV_KEY_REBOOT_INFO, blob);

    if(read_len == sizeof(RebootPersistentInfo_t) && info.magic == REBOOT_MAGIC_NUM)
    {
        if(reason_buf != NULL && buf_len > 0)
        {
...
static void reboot_clear_persistent_info(void)
{
    fdb_kv_del(&net_kvdb, KV_KEY_REBOOT_INFO);
...
{ 
    ...
    else if(strcmp(type->valuestring, "config_set") == 0)
    {
        ...
        if(tcp_pcb != NULL)
        {
            LOG("Server IP changed, abort old TCP connection immediately");
            tcp_close(tcp_pcb);
            tcp_pcb = NULL;
            g_tcp_connected = 0;
            hb_lost_cnt = 0;
...
static void tcp_send_str(struct tcp_pcb *tpcb, const char *str)
{
    ...
    if (tcp_send_mutex == NULL)// 进入临界区前先校验锁句柄
    {
    ...
    ret = tcp_write(tpcb, send_buf, json_len + 1, TCP_WRITE_FLAG_COPY);
    if(ret != ERR_OK)
    {
...
void tcp_send_execute_result(ExecuteResultInfo_t *info_arr, uint16_t arr_len)
{
    ...
    if(str)
    {
        // osMutexAcquire(tcp_send_mutex, portMAX_DELAY);
        tcp_send_str(tcp_pcb, str);
        // osMutexRelease(tcp_send_mutex);
        LOG("TCP Send JSON:\r\n%s\r\n", str);
        cJSON_PortFree(str);
...
void tcp_send_device_reboot_ack(uint8_t status, const char *message, uint32_t estimated_duration)
{
    ...
    {
        tcp_send_str(tcp_pcb, str);
        LOG("TCP Send device_reboot_ack JSON:\r\n%s\r\n", str);
        cJSON_PortFree(str);
        g_tcp_seq++;
    }
...
void tcp_send_device_reboot_result(uint8_t result, const char *message, const char *reason, uint32_t uptime)
{
    ...
    if(send_buf)
    {
        LOG("TCP Send device_reboot_result:\r\n%s\r\n", send_buf);
        tcp_send_str(tcp_pcb, send_buf);
...
void vRebootTask(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        // if(main_sys_status.is_Busy)
        // {
        //     tcp_send_device_reboot_ack(2, "device_busy", 0);
        //     LOG("Reject reboot: device busy, reason:%s\r\n", cmd.reason);
        //     continue;
        // }

        // 2. 接受重启，先发送应答，保证上位机收到
