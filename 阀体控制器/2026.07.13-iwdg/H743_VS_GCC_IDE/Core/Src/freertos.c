...
const osThreadAttr_t RebootTask_attributes = {
  .name       = "RebootTask",
  .stack_size = 256 * 4,    // 栈大小1024字节，兼容Flash读写、TCP发送、字符串操作
  .priority   = osPriorityNormal1,  // 优先级低于核心业务任务，不抢占注油/ADC
};
...
void ETH_InitTask(void *argument)
{
  ...
  Initialization_time = (HAL_GetTick() - start + 5000)/1000; // 预留5秒，确保系统初始化完成
...
void ADCTask(void *argument)
{
    LOG_ADC("buf_addr = 0x%p", adc_raw_buf);
    LOG_ADC("ADC3_CR   = 0x%08x", ADC3->CR);
    LOG_ADC("ADC3_CFGR = 0x%08x", ADC3->CFGR);
    for (;;)
    {
      LOG_ADC("CNDTR=%d raw0=%d raw1=%d raw2=%d raw3=%d",
          BDMA_Channel0->CNDTR,
