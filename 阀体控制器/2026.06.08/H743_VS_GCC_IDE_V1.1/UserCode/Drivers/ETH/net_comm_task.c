...
void FillHeartBeatCommonData(HeartBeatParam_t *hb_param);
void tcp_send_state_response(HeartBeatParam_t *p_param);
    ...
    while(1)
    {
        ...
        if(tcp_pcb != NULL && g_tcp_connected == 1 && g_network_configured == 1)
        {
            ...
            if(bSendHeartbeatFlag)
            {
                ...
                // // 填充参数
                // HeartBeatParam_t rsp_param = {0};
                // FillHeartBeatCommonData(&rsp_param);
                // // 发送心跳
                // tcp_send_heartbeat(&rsp_param);
                TaskInfo_t hb_msg = {0};
                hb_msg.inj_id = 0; // 约定inj_id=0 = 心跳报文标识
                xQueueSend(xTimerReqQueue, &hb_msg, 0U);
            }
        }
...
static void tcp_send_str(struct tcp_pcb *tpcb, const char *str)
{
    ...
    if(free_buf < strlen(str))//剩余发送空间小于报文长度，直接丢包
    {
        LOG("tcp send buf full,drop pkt\r\n");
        return;
    }
    err_t ret = tcp_write(tpcb, str, strlen(str), TCP_WRITE_FLAG_COPY);
    // tcp_write失败不再强行tcp_output，防止队列错乱断言
    if(ret == ERR_OK)
    {
        tcp_output(tpcb);
    }
    ...
    {
        LOG("tcp_write fail,err:%d",ret);
        // 发送内存失败直接断连接，触发重连释放全部发送队列
        tcp_abort(tpcb);
        tcp_pcb = NULL;
        g_tcp_connected = 0;
        bSendHeartbeatFlag = 0;
...
static void tcp_send_heartbeat(HeartBeatParam_t *p_param)
{
    ...
    {
        cJSON *item = cJSON_CreateObject();
        InjectorItem_t *p_inj = &p_param->injectors[i];
        cJSON_AddNumberToObject(item, "inj_id", p_inj->inj_id);
        cJSON_AddNumberToObject(item, "status", p_inj->status);//数字
        cJSON_AddNumberToObject(item, "last_error_code", p_inj->last_error_code);
        cJSON_AddStringToObject(item, "last_error_msg", p_inj->last_error_msg);
...
void tcp_send_state_response(HeartBeatParam_t *p_param)
{
    ...
    for(int i = 0; i < INJECTOR_CNT; i++)
    {
        ...
        InjectorItem_t *p_inj = &p_param->injectors[i];
        cJSON_AddNumberToObject(item, "inj_id", p_inj->inj_id);
        cJSON_AddNumberToObject(item, "status", p_inj->status);
        cJSON_AddNumberToObject(item, "last_error_code", p_inj->last_error_code);
        cJSON_AddStringToObject(item, "last_error_msg", p_inj->last_error_msg);
        cJSON_AddItemToArray(inj_arr, item);
    }
    ...
    cJSON *cap_copy = cJSON_Duplicate(cap, 1);
    cJSON *inj_copy = cJSON_Duplicate(inj_arr, 1);
...
// 填充state_response/heartbeat共用数据
void FillHeartBeatCommonData(HeartBeatParam_t *hb_param)
{
    ...
    hb_param->dev_status.current_inj_id = 0;
    strncpy(hb_param->dev_status.current_task_id, g_task_id, sizeof(hb_param->dev_status.current_task_id)-1);
    hb_param->dev_status.current_task_start = 0;
    hb_param->dev_status.estimated_complete = 0;

    for(int i = 0; i < INJECTOR_CNT; i++)
    {
        _INJECTOR_INFO *p_inj = &main_sys_status.injector[i];
        InjectorItem_t *p_item = &hb_param->injectors[i];
        ...
        // capability数组字段
        HeartCapInfo_t *p_cap = &hb_param->cap_arr[i];
        p_cap->inj_id = p_inj->injector_id;
        p_cap->inj_vs = p_inj->volume;
        // 单位转换：源码秒 → 协议ms
        p_cap->inj_tr = (uint32_t)p_inj->executionTime * 1000U;  //单次喷油耗时 s→ms
        p_cap->inj_ts = (uint32_t)p_inj->interval * 1000U;       //配置周期 s→ms

        //cap.inj_st：0空闲，1注油中，2故障
        if(p_inj->injectRequest == 98)
        {
            p_cap->inj_st = 2;
        }
        else if(main_sys_status.is_Busy == 1 && p_inj->injectRequest ==99)
        {
            p_cap->inj_st = 1;
        }
        else
...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    if(strcmp(type->valuestring, "task_order") == 0)
    {
        ...
        for(int i = 0; i < g_task_order_cnt; i++)
        {
            ...
            // 收到有效任务ID存入全局g_task_id（心跳上报使用）
            memset(g_task_id,0,sizeof(g_task_id));
            strncpy(g_task_id, g_task_order_buf[i].task_id, sizeof(g_task_id)-1);
        }
...
void tcp_send_execute_result(ExecuteResultInfo_t *info_arr, uint16_t arr_len)
{
    ...
    for (uint16_t i = 0; i < arr_len; i++)
    {
        ...
        //失败才追加错误信息，成功不生成这两个key
        if(info_arr[i].execute_result == 0)
        {
    ...
    while(1)
    {
        ...
        if(xQueueReceive(xTcpTaskQueue, &recv_item, portMAX_DELAY) == pdPASS)
        {
            ...
            if(task_object_list[idx].task_status == 0)
            {
                ...
                LOG("TCP task write slot3 ok: inj=%d vol=%d\r\n",recv_item.inj_id,recv_item.inj_v);
            }
            else
            {
                LOG("slot3 busy,discard tcp task inj=%d vol=%d\r\n",recv_item.inj_id,recv_item.inj_v);
            }
        }
    }
}
    ...
    for(;;)
    {
        if(xQueueReceive(xTimerReqQueue, &item, pdMS_TO_TICKS(50)) == pdPASS)
        {
            ...
            else// 定时喷油上报
            {
                ...
                tcp_send_task_request(&item, 1, busy);
            }
            //单发完延时200ms，隔开每条上报
...
void TimeSyncTask(void *arg)
{
    ...
    const uint32_t CHECK_INTERVAL      = 2000;  // 校验周期 2秒
