...
#define LOG_ENABLE  0   // 1: 开启打印, 0: 关闭打印

#if LOG_ENABLE
#define LOG(fmt, ...) printf("[R] " fmt "\r\n", ##__VA_ARGS__)
#else
...
typedef struct
{
    ...
    uint32_t current_task_start;//当前任务开始时间，单位ms，Unix时间戳
    uint32_t estimated_complete;//预计完成时间，单位ms，Unix时间戳
}DevStatus_t;
...
typedef struct
{
    uint32_t inj_acvs;          //实际注油量
    uint16_t inj_id;            //当前注油器ID，1-8
    // uint32_t inj_jr;         //今日注油次数
    uint8_t  status;            //0=正常,1=禁用,2=故障
    uint32_t inj_ts;            //注油间隔
    uint32_t inj_vs;            //单次注油量
    uint32_t progress_ts;       //距离下次注油剩余倒计时秒数
    uint16_t last_error_code;   //最后错误代码，0表示无错误
    char last_error_msg[48];    //最后错误信息
}InjectorItem_t;
...
typedef struct
{
    char task_id[48];
    uint16_t inj_id;
    uint32_t inj_ts;    // 新增字段：对应报文 inj_ts
    uint32_t inj_v;
} TaskOrderInfo_t;
...
typedef struct
{
    ...
    uint32_t valve_act_tm;     // 阀体单次工作耗时(ms)
    uint32_t valve_total_tm;   // 阀体工作总耗时(ms)
...
typedef struct
{
    char task_id[48];    // TCP下发的任务编号
    uint8_t bind_inj_id; // 该task_id绑定的注油口ID，0=无有效绑定
} TaskBindInfo_t;
...
extern volatile uint8_t g_force_stop_task;//强制停止标记
extern ExecuteResultInfo_t exec_info;//任务结果上报
extern TaskBindInfo_t g_tcp_task_bind;
