...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    if(strcmp(type->valuestring, "stop_task") == 0)
    {
        ...
        if(strlen(g_task_id) != 0 && strcmp(g_task_id, g_stop_target_task_id) == 0)
        {
            g_force_stop_task = 1;
        ...
        {
            LOG("stop_task task_id mismatch, current run:%s, req stop:%s\r\n", g_task_id, recv_stop_taskid);
        }
    }
...
void test_oil_filling_task(void *arg)
{
    ...
    for(;;)
    {
        ...
        while(fill_cur < fill_total)
        {
            ...
            strncpy(exec_info.task_id, g_task_id, sizeof(exec_info.task_id)-1);
