...
void PrintTaskStackAndCount(void)
{
    ...
    LOG("7 ,syncif_task_cycle         min stack left: %lu bytes", uxTaskGetStackHighWaterMark(syncif_task_handler_cycle));
    LOG("8 ,mbif_task                 min stack left: %lu bytes", uxTaskGetStackHighWaterMark(mbif_task_handler));
    LOG("9 ,injector_task             min stack left: %lu bytes", uxTaskGetStackHighWaterMark(injector_task_handler));
    LOG("10,ETH_InitTask              min stack left: %lu bytes", uxTaskGetStackHighWaterMark(ETH_InitTaskHandle));
    LOG("11,udp_discover_task         min stack left: %lu bytes", uxTaskGetStackHighWaterMark(udpTaskHandle));
    LOG("12,tcp_client_task           min stack left: %lu bytes", uxTaskGetStackHighWaterMark(tcpConfigTaskHandle));
    LOG("13,TCP_task_processing_task  min stack left: %lu bytes", uxTaskGetStackHighWaterMark(TCP_task_processingTaskHandle));
    LOG("14,TimerReqSendTask          min stack left: %lu bytes", uxTaskGetStackHighWaterMark(TimerReqSendTaskHandle));
    LOG("15,TimeSyncTask              min stack left: %lu bytes", uxTaskGetStackHighWaterMark(TimeSyncTaskHandle));
    LOG("16,RebootTask                min stack left: %lu bytes", uxTaskGetStackHighWaterMark(RebootTaskHandle));
    LOG("17,TcpSenderTask             min stack left: %lu bytes", uxTaskGetStackHighWaterMark(TcpSenderTaskHandle));
...
void PrintTaskStackAndCount_2(void)
{
    ...
    for(uint32_t i = 0; i < TASK_MONITOR_NUM; i++)
    {
        ...
        {
            LOG("%-25s Handle=NULL", p_item->task_name);
            continue;
        }
        ...
        {
            LOG("%-25s min stack left: %lu words / %lu bytes, drop_cnt:%lu",
                p_item->task_name, curr_water, curr_byte, (unsigned long)p_item->drop_cnt);
        }
        else
        {
            LOG("%-25s min stack left: %lu words / %lu bytes",
        ...
        {
            LOG("[WARN] STACK LOW! Task:%s remain words:%lu", p_item->task_name, curr_water);
        }
        ...
        if(p_item->last_watermark != 0U)
        {
            if(curr_water < p_item->last_watermark)
            {
                ...
                {
                    LOG("[CRITICAL] STACK CONTINUE DROP! Task:%s last:%lu curr:%lu, drop_cnt:%lu",
...
#define HEAP_ALERT_THRESHOLD    (4*1024U)
    ...
    {
        LOG("[WARN] HEAP LOW WARNING! min free heap:%u", heap_free_min);
    }
