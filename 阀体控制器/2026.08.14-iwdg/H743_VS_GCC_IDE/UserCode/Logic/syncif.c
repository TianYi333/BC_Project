...
void start_syncif_cycle_task() {
  syncif_task_handler_cycle = osThreadNew(syncif_task_cycle, NULL, &syncif_task_attributes);
}
  ...
  if (main_sys_status.sys_start != 200) {
    for (tag = 0; tag < injector_count; tag++) {
      if (main_sys_status.injector[tag].interval > 0) {
        ...
        if (xRemainingTime != 0 && xRemainingTime > xTaskGetTickCount()) 
        {
    ...
    for (tag = 0; tag < injector_count; tag++) {
      ...
      main_sys_status.injector[tag].executionTime = 0; // 停机时清空剩余倒计时
    }
  }
}
...
{
    struct fdb_blob blob;
    fdb_kv_set_blob(&kvdb, "injector_config", fdb_blob_make(&blob, cfg, sizeof(_INJECTOR_CONFIG)));
}
