...
void StartDefaultTask(void *argument)
{
  ...
  start = HAL_GetTick();
  LOG_Debug("init_net_db total start time: %lu ms", start);
  //—————————————必须放在MX_LWIP_Init();之后———————————————————————
  init_sys_db();//void syncif_task(void *argument)任务里的已经注释掉
  LOG_Debug("init_sys_db run time: %lu ms", HAL_GetTick() - start);
  init_net_db();
  LOG_Debug("init_net_db total run time: %lu ms", HAL_GetTick() - start);
...
void PrintTaskStackAndCount(void)
{
    ...
    LOG("1 ,ADCTask                   min stack left: %u bytes", uxTaskGetStackHighWaterMark(ADCTaskHandle));
...
void ETH_InitTask(void *argument)
{
  ...
  //   }
  // }
  LOG_Debug("init run time: %lu ms", HAL_GetTick() - start);
...
void ADCTask(void *argument)
{
    ...
    for (;;)
    {
      ...
      LOG_Debug("CNDTR=%d raw0=%d raw1=%d raw2=%d raw3=%d",
          BDMA_Channel0->CNDTR,
          adc_raw_buf[0], adc_raw_buf[1], adc_raw_buf[2], adc_raw_buf[3]);
      vTaskDelay(500);
    }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    ...
    if(hadc->Instance == ADC3)
    {
        ...
        {
            float voltage = adc_raw_buf[i] * 3.3f / 65535.0f;
            adc_current[i] = voltage / 160.0f * 1000.0f;
        }
    }
}
