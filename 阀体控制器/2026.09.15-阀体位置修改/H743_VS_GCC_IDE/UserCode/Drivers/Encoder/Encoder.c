...
// 编码器引脚电平基准（文件级，确保上电即与实际引脚电平同步，避免首按漏检）
static uint8_t lastA = 0;
static uint8_t lastB = 0;
static uint8_t lastP = 0;
static uint8_t g_encoder_synced = 0; // 0:未同步 1:已同步实际电平

/**
 * @brief 同步编码器引脚初始电平到内部基准
 * @note 应在 OLED 任务启动、用户首次操作前调用一次，确保 lastA/lastB/lastP
 *       与引脚实际空闲电平一致（EC11_P 无内部上下拉，空闲为高），否则上电后
 *       第一次按键会因边沿判定基准错误而被漏检，需按两次才生效。
 */
void Encoder_SyncInitialLevel(void)
{
    lastA = HAL_GPIO_ReadPin(ENCODER_A_PORT, ENCODER_A_PIN);
    lastB = HAL_GPIO_ReadPin(ENCODER_B_PORT, ENCODER_B_PIN);
    lastP = HAL_GPIO_ReadPin(ENCODER_P_PORT, ENCODER_P_PIN);
    g_encoder_synced = 1;
}
...
{
  // 首次进入时同步引脚当前实际电平（兜底：防止未调用 Encoder_SyncInitialLevel 时首按漏检）
  if (g_encoder_synced == 0)
  {
    lastA = HAL_GPIO_ReadPin(ENCODER_A_PORT, ENCODER_A_PIN);
    lastB = HAL_GPIO_ReadPin(ENCODER_B_PORT, ENCODER_B_PIN);
    lastP = HAL_GPIO_ReadPin(ENCODER_P_PORT, ENCODER_P_PIN);
