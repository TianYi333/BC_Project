...
extern QueueHandle_t msgQueue_ID_CAN1; /* CAN1消息队列ID */
extern QueueHandle_t msgQueue_ID_CAN2; /* CAN1消息队列ID */
/* 内部变量 */
...
_FDCAN_HANDLER fdcan1 = { .rx_MSG = { 0 }, .start = FDCAN1_Start, .rx_Filter_Init = FDCAN1_RX_Filter0_Init,
		.rx_Interrupt_Init = FDCAN1_Interrupt_Init, .send_MSG = FDCAN1_Send_Msg, .update_RXFIFO_Status =
				update_RXFIFO_Status, };
_FDCAN_HANDLER fdcan2 = { .rx_MSG = { 0 }, .start = FDCAN2_Start, .rx_Filter_Init = FDCAN2_RX_Filter0_Init,
		.rx_Interrupt_Init = FDCAN2_Interrupt_Init, .send_MSG = FDCAN2_Send_Msg, .update_RXFIFO_Status =
				update_RXFIFO_Status, };
...
void FDCAN1_RX_Filter0_Init(void) {
	hfdcan1_RX_Filter0.IdType = FDCAN_EXTENDED_ID; /* 只接收标准帧ID */
	...
	hfdcan1_RX_Filter0.FilterID1 = 0x00000000; /* 滤波ID1: 0x00 */
	hfdcan1_RX_Filter0.FilterID2 = 0x00000000; /* 滤波ID2: 0x00 */
	 ...
	 */
	if (HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_ACCEPT_IN_RX_FIFO0, DISABLE, DISABLE)
			!= HAL_OK) /* 设置FDCAN1滤波器0全局配置  */
			{
		user_Assert(__FILE__, __LINE__);
	}
}
...
void FDCAN2_RX_Filter0_Init(void) {
	hfdcan2_RX_Filter0.IdType = FDCAN_EXTENDED_ID; /* 只接收标准帧ID */
	 ...
	 *  2.
	 *  3.如果启用RXFIFO0,那么滤波器必须关联到RXFIFO0,即FilterConfig必须赋值	.
	 *  4.同理,如果启动RXFIFO1的话,滤波器必须关联到RXFIFO1,即FilterConfig必须赋值FDCAN_FILTER_TO_RXFIFO1.
	 *
	 */
	hfdcan2_RX_Filter0.FilterConfig = FDCAN_FILTER_TO_RXFIFO1; /* 滤波器关联到RXFIFO1 */
	hfdcan2_RX_Filter0.FilterID1 = 0x00000000; /* 滤波ID1: 0x00 */
	hfdcan2_RX_Filter0.FilterID2 = 0x00000000; /* 滤波ID2:   0x00 */
	 ...
	 * 参数5：设置是否拒绝远程拓展帧，ENABLE代表拒绝接收。
	 */
	if (HAL_FDCAN_ConfigGlobalFilter(&hfdcan2, FDCAN_ACCEPT_IN_RX_FIFO1, FDCAN_ACCEPT_IN_RX_FIFO1, DISABLE, DISABLE)
			!= HAL_OK) /* 设置FDCAN1滤波器1全局配置  */
			{
		user_Assert(__FILE__, __LINE__);
	}
}
...
void FDCAN2_Interrupt_Init(void) {
	...
	if (HAL_FDCAN_ActivateNotification(&hfdcan2, FDCAN_IT_RX_FIFO1_FULL, 2) != HAL_OK) /* 使能FDCAN中断(接收到新数据) */
	{
		user_Assert(__FILE__, __LINE__);
	}
}
...
uint8_t FDCAN1_Send_Msg(_CANMSG *msg) {
	...
	hfdcan1_TX.IdType = FDCAN_EXTENDED_ID; /* 标准ID */
	...
	if (REMOTE_FRAME == msg->rtr)
		...
		hfdcan1_TX.TxFrameType = FDCAN_DATA_FRAME; /* 数据帧 */
//-------------------------------------------------------------------------------------------------------------------------------
    // 先检查 FIFO 空闲，避免 HAL 内部死循环
    if(HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1) == 0) {
        return 0;   // FIFO 满，直接返回失败
    }
//-------------------------------------------------------------------------------------------------------------------------------
	/* 将需要发送的数据压入到TX FIFO */
...
uint8_t FDCAN2_Send_Msg(_CANMSG *msg) {
	...
	hfdcan2_TX.IdType = FDCAN_EXTENDED_ID; /* 标准ID */
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs) {
	...
	if (FDCAN_IT_RX_FIFO0_WATERMARK == RxFifo0ITs || FDCAN_IT_RX_FIFO0_NEW_MESSAGE == RxFifo0ITs
		...
		do {
			...
			xQueueSendFromISR(msgQueue_ID_CAN1, &fdcan1.rx_MSG, NULL);
...
void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo1ITs) {
	...
	/* FIFO1新数据中断 */
	if (FDCAN_IT_RX_FIFO1_WATERMARK == RxFifo1ITs || FDCAN_IT_RX_FIFO1_NEW_MESSAGE == RxFifo1ITs
		...
		do {
			...
			/* 将CAN报文放入消息队列 */
			xQueueSendFromISR(msgQueue_ID_CAN2, &fdcan2.rx_MSG, NULL);
