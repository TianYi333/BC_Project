...
MenuItem system_settings_menu_items[] = {
    ...
    {"P Model Speed", p_model_speed_settings_callback},
    {"Q Model Speed", q_model_speed_settings_callback},
...
void show_temperature_humidity_page(u8g2_t *u8g2)
{
    ...
    do
    {
        ...
        char sn_buf[32] = {0};
        snprintf(sn_buf, sizeof(sn_buf), "ID:%s", g_device_sn);
        // 靠右显示，距离屏幕右边缘10像素，Y基线和P/Q/S保持39
        uint16_t sn_width = u8g2_GetStrWidth(u8g2, sn_buf);
        uint16_t sn_x = OLED_WIDTH - sn_width - 10;
        u8g2_DrawStr(u8g2, sn_x, 39, sn_buf);
        // ==============================================
    ...
    while (!long_pressed)
    {
        if (xQueueReceive(QUEUE_KEY, &encoder_state, pdMS_TO_TICKS(200)) == pdPASS)
        {
            ...
            case ENCODER_PUTH:
                ...
                p_motor_run_point(motor_loc_list_p[p_step]);
                p_step++;
                break;
            case ENCODER_CW:
                if (q_step > 7)
