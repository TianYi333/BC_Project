 ...
 * 注油器任务的相关定义
 */
#define injector_task_stk_size     1024*8
...
typedef struct injector_info {
	//注油器ID,从1开始编号，最大值为injector_count
	uint16_t injector_id;
	//注油量，单位为毫升
	uint32_t volume;
	//注油动作的间隔时间
	uint64_t interval;
	//注油动作的执行时间,表征当前点位的待机进度
	uint64_t executionTime;
	//注油动作的执行量,表征当前点位注油进度
	uint32_t executionVol;
	//注油动作的请求量,表征当前点位的注油需求
	uint8_t injectRequest;
	//当前任务状态，0表示无任务，1表示待执行，2表示执行中，3表示成功，4表示失败
	uint8_t task_status;
    //注油动作的性能指标,表征当前点位的注油效率
	uint32_t execution_performance;
	//任务要求的注油量
	uint32_t task_volume;
	//注油点开关
	uint8_t enable;
	// 新增：当前注油实时进度，未注油时为0
	volatile uint32_t current_progress;
} _INJECTOR_INFO;
/**
 ...
 * 电机位置列表
 */
extern __attribute__((section("._DTC_FSRAM"))) _MOTOR_LOC motor_loc_list[12];

/**
 * 六代阀P模块定位
 */
extern __attribute__((section("._DTC_FSRAM"))) _MOTOR_LOC motor_loc_list_p[12] ;

/**
 * 主要系统状态
 */ 
extern __attribute__((section("._DTC_FSRAM"))) _SYS_STATUS main_sys_status;


// 单路注油口错误信息（仅错误码、错误描述）
typedef struct injector_err_info {
    uint8_t err_code;         // 错误码
    char err_msg[48];         // 错误文字描述
} _INJECTOR_ERR_INFO;

extern __attribute__((section("._DTC_FSRAM"))) _INJECTOR_ERR_INFO injector_err_list[injector_count];// 全局故障数组，共injector_count路，存放在不掉电RAM
