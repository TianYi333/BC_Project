...
static void FillSingleInjectorItem(InjectorItem_t *p_item, uint16_t port_idx)
{
    ...
    p_item->progress_ts = p_inj->executionTime;//转发定时器剩余倒计时到上报结构体 progress_ts
...
//             }
            ...
            if(recv_item.inj_ts > 0)
            {
                ...
                if(inj_id >= 1 && inj_id <= injector_count)
                {
                    ...
                    {
                        logError("TCP invalid interval inj%d, ts_sec:%llu\r\n", inj_id, new_sec);
                    }
                }
                else
                {
                ...
                if(inj_id >= 1 && inj_id <= injector_count)
                {
                    ...
                    if(new_volume >= VOLUME_MIN && new_volume <= VOLUME_MAX)
                    {
                        if(injector_config.injector[tag].volume != new_volume)
                        {
                            ...
                            LOG("TCP update injector%d config volume to %lu mL\r\n", inj_id, new_volume);
                            config_changed = 1;
                        }
                    }
                    else
                    {
                        logError("TCP invalid volume inj%d, volume:%lu\r\n", inj_id, new_volume);
                    }
                }
                else
                {
                    LOG("TCP modify volume invalid inj_id:%d\r\n", inj_id);
                }
            }
