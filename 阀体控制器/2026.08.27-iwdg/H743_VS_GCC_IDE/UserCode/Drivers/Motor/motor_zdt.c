...
{
	uint16_t ex_addr = addr * 256;
	 _CANMSG MOTOR_MsgStructure = {0}; // 全部字段清零，避免栈随机值
	//MOTOR_MsgStructure.id = ex_addr + 1; // 发送命令ID = ex_addr + 1（和定位函数保持一致规则）
	MOTOR_MsgStructure.id = ex_addr; // 发送命令ID = ex_addr + 1（和定位函数保持一致规则）
	MOTOR_MsgStructure.len = 4;
