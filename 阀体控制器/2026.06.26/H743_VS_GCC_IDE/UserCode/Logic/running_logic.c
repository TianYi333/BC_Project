...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    exec_info.error_code = Q_MODEL_RESET_FAILURE;//错误码
    strncpy(exec_info.error_msg, "Q_MODEL_RESET_FAILURE", sizeof(exec_info.error_msg)-1);
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    exec_info.valve_hum = 38;
    exec_info.execute_result = 3;//错误
    exec_info.error_code = P_MODEL_RESET_FAILURE;//错误码
    strncpy(exec_info.error_msg, "P_MODEL_RESET_FAILURE", sizeof(exec_info.error_msg)-1);
    exec_info.error_msg[sizeof(exec_info.error_msg)-1] = '\0';
...
//-------------------------------单次上报------------------------------------------------------------------------------------------------
    ...
    // 单条进度上报
    tcp_send_execute_result(&exec_info, 1U);
    printf("Filling progress: current %d mL / total %d mL\r\n", fill_cur, inject_val);
//-------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------成功上报----------------------------------------------------------------------------------------------
    ...
    //注油结束，清空全局任务ID
    memset(&g_tcp_task_bind, 0, sizeof(g_tcp_task_bind));
    printf("Oil filling finished, inj_id:%d, actual total vol:%d mL, total run time:%d ms\r\n",injector_id, fill_cur, action_time);
