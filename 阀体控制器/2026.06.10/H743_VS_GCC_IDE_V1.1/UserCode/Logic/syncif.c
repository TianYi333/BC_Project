...
static struct fdb_default_kv_node default_kv_table[] = {
	...
	{"slaveID", &main_sys_status.slaveID, sizeof(main_sys_status.slaveID)}, 
	{"injector_info", main_sys_status.injector, sizeof(main_sys_status.injector[0]) * injector_count}};
/* KVDB object */
	...
	if (main_sys_status.sys_start == 200)
	{
		for (tag = 0; tag < injector_count; tag++)
		{
			if (main_sys_status.injector[tag].interval > 0)
			{
				...
				xRemainingTime = xTimerGetExpiryTime(xTimeHandle[tag]);//定时器下次到期那一刻的全局系统绝对 Tick 值
				if (xRemainingTime != 0)
				{
