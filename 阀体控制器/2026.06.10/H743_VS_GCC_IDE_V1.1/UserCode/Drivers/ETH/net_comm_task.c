...
void tcp_client_task(void *arg)
{
    ...
    if(!bTimerInit)
    {
        ...
        //修改所有软件定时器的优先级为高，确保心跳定时器回调能及时执行，避免网络不稳定时心跳发送延迟过长导致误判断连
        osThreadSetPriority(xTimerGetTimerDaemonTaskHandle(), osPriorityAboveNormal);//CMSIS API，用语义化枚举
        //vTaskPrioritySet(xTimerGetTimerDaemonTaskHandle(), 40);//原生API，直接设原生优先级
    }
...
void FillStateRespData(StateRespParam_t *resp_param, uint16_t port_idx)
{
    ...
    resp_param->dev_status.current_inj_id = 0;
    strncpy(resp_param->dev_status.current_task_id, g_task_id, sizeof(resp_param->dev_status.current_task_id) - 1);
...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    if(strcmp(type->valuestring, "state_request") == 0)
    {
        ...
        //填充指定端口数据
        FillStateRespData(&resp_param, req_inj_id);
        //发送状态响应报文
...
void TCP_task_processing_task(void *argument)
{
    ...
    while(1)
    {
        ...
        if(xQueueReceive(xTcpTaskQueue, &recv_item, portMAX_DELAY) == pdPASS)
        {
            ...
            {
                LOG("System busy, discard tcp task inj=%d vol=%d\r\n",
            ...
            if(task_object_list[idx].task_status == 0)
            {
                ...
                strncpy(g_task_id, recv_item.task_id, sizeof(g_task_id) - 1);
            }
            else
            {
        ...
        if(xQueueReceive(xTimerReqQueue, &item, pdMS_TO_TICKS(50)) == pdPASS)
        {
           ...
           // 聚合窗口：500ms 收集短时内所有后续任务
            ...
            if(g_task_request_cnt > 0)
            {
                ...
                uint8_t busy = 0;//不区分系统忙闲，定时上报任务请求，确保上位机及时收到注油请求，提升响应速度
                // 批量发送：传数组首地址 + 有效数量
