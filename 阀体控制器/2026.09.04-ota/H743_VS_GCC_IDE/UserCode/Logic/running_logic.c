...
void injector_task() {
  ...
  while (1) {
    ...
    case SYS_MODE_STARTING:
      ...
      start_running();
      main_sys_status.is_Busy = 0;  /* 提前复位，防止被设置模式抢占后卡住 */
      if (main_sys_status.running_mode != SYS_MODE_SETTING) {
        vTaskDelay(2000);           /* 被抢占则不延时，立即让位给设置页 */
      }
      break;
      /**
...
/**
 * @brief 仅当仍处于启动流程时才允许切换模式，被设置模式抢占则不覆盖
 */
static void set_mode_if_starting(_SYS_MODE mode) {
  if (main_sys_status.running_mode == SYS_MODE_STARTING) {
    main_sys_status.running_mode = mode;
  }
}
...
uint8_t start_running(void) {
  ...
  /* P自检阻塞期间可能被设置模式抢占，检查后再继续 */
  if (main_sys_status.running_mode != SYS_MODE_STARTING) return 0;
  /**
    ...
    if (q_motor_run_point(motor_loc_list[0]) == 1) {
      ...
      if (waiting_met_sen(motor_loc_list[0].pipe) > 0) {
        ...
        if (q_motor_run_point(motor_loc_list[1]) != 1) {
          /* Q电机阻塞期间可能被抢占，中止且不覆盖模式 */
          if (main_sys_status.running_mode != SYS_MODE_STARTING) return 0;
          /**
           * 电机运行到预备点失败，报错返回
           */
          main_sys_status.sys_warning = Q_MODEL_RESET_FAILURE;
          set_mode_if_starting(SYS_MODE_STARTING);
          ...
          set_mode_if_starting(SYS_MODE_READY);
          return 1;
        }
      } else {
        /* waiting_met_sen阻塞期间可能被抢占，中止且不覆盖模式 */
        if (main_sys_status.running_mode != SYS_MODE_STARTING) return 0;
        /**
         * 传感器确认失败，报错返回
         */
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    LOG_logic("Oil filling FAIL, inj_id:%u, err_code:%u, msg:%s\r\n", injector_id, exec_info.error_code, exec_info.error_msg);
    // 记录当前出油口故障
...
//-------------------------------单次上报------------------------------------------------------------------------------------------------
    ...
    LOG_logic("Filling progress: current %lu mL / total %lu mL\r\n", (unsigned long)fill_cur, (unsigned long)inject_val);
...
//---------------------------------成功上报----------------------------------------------------------------------------------------------
  ...
  //注油结束，清空全局任务ID
  memset(&g_tcp_task_bind, 0, sizeof(g_tcp_task_bind));
  LOG_logic("Oil filling finished, inj_id:%u, actual total vol:%lu mL, total run time:%lu ms\r\n",
