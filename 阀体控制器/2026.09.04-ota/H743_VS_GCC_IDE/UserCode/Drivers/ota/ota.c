...
static int ota_feed_chunk(const uint8_t *data, uint16_t len)
{
    ...
    /* 写入 firmware_a 分区（SPI Flash），flash_kv_mutex 串行化 */
    if (flash_kv_mutex != NULL)
    {
        osMutexAcquire(flash_kv_mutex, osWaitForever);
    }
    int wr = fal_partition_write(g_fw_part, g_ota_off, data, len);
    if (flash_kv_mutex != NULL)
    {
    ...
    if (wr != (int)len)
    {
        LOG_OTA("ota: fal write fail off=%lu\r\n", (unsigned long)g_ota_off);
        return -1;
    }
...
static void ota_on_complete(void)
{
    ...
    fal_partition_erase(g_meta_part, 0, OTA_META_SIZE);
    fal_partition_write(g_meta_part, 0, (const uint8_t *)&meta, sizeof(meta));
    if (flash_kv_mutex != NULL)
    {
        osMutexRelease(flash_kv_mutex);
    }
...
int ota_handle_upgrade_start(uint32_t new_len, uint32_t new_crc32)
{
    ...
    if (new_len == 0 || new_len > OTA_APP_MAX_SIZE)
    {
        LOG_OTA("ota: invalid new_len=%lu (max %lu)\r\n",
                (unsigned long)new_len, (unsigned long)OTA_APP_MAX_SIZE);
        return -2;  /* 长度非法（0 或超过内部 Flash BANK2 容量 512KB） */
    }
    if (new_crc32 == 0)
    {
...
void ota_app_confirm(void)
{
    ...
    if (meta.magic != OTA_META_MAGIC || meta.state != OTA_STATE_DONE)
    {
        /* 非 DONE（NONE/CONFIRMED/PENDING/FAILED）：无需确认 */
        return;
    }
