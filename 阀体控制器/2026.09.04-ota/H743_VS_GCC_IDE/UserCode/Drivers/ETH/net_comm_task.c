...
static volatile uint8_t tcp_pending_close = 0;      // TCP异步关闭标记，避免lwIP内核阻塞互斥锁
...
{ 
    ...
    else if(strcmp(type->valuestring, "config_set") == 0)
    {
        ...
        if(tcp_pcb != NULL)
        {
            ...
            // 注意：reconnect_tick 随后会被 tcp_client_task 的 pending-close 分支重置为
            // HAL_GetTick()，仅靠回拨tick无法生效，需用独立标记让重连门跳过节流
            g_fast_reconnect = 1;
        }
...
{
    // 先获取lwIP核心锁，再拿tcp_send_mutex；与tcp_sender_task保持一致顺序(core->mutex)，避免死锁
    LOCK_TCPIP_CORE();
...
void tcp_client_task(void *arg)
{
    ...
    while(1)
    {
        ...
        if(tcp_pcb == NULL)// TCP自动重连
        {
            ...
            g_fast_reconnect = 0; // 消费快速重连标记：本次重连跳过节流，后续恢复正常节流

            // 在lwIP核心线程保护下创建/连接PCB，防止与tcpip_thread并发操作pcb链表
            LOCK_TCPIP_CORE();
            // 二次加锁防护，防止多路径创建PCB冲突（核心锁已持有，不可再调safe_tcp_close）
            if(tcp_pcb != NULL)
            {
                UNLOCK_TCPIP_CORE();
                continue;
            ...
            if(new_pcb != NULL)
            {
                ...
                if(ret != ERR_OK)
                {
                    // SYN_SENT状态下tcp_close可能因发送缓存不足失败，用tcp_abort确保PCB回收
                    tcp_abort(new_pcb);
                    new_pcb = NULL;
                    g_tcp_connected = 0;
                    stable_tick = 0;
...
static void tcp_error_callback(void *arg, err_t err)
{
    ...
    // 防止后续safe_tcp_close对已经释放的pcb再次调用tcp_close/tcp_abort造成HardFault/pbuf损坏
    tcp_pcb = NULL;
    g_tcp_connected = 0;
...
/**
 * @brief TCP连接建立回调，互斥锁10ms超时，防止阻塞lwIP内核线程
 */
static err_t tcp_connected_cb(void *arg, struct tcp_pcb *tpcb, err_t err)
{
    if(err != ERR_OK)
...
//         LOG_NET("ERROR: tcp_send_mutex is null, skip send\r\n");
//         return;
//     }
...
//         LOG_NET("tcp send buf full, drop pkt\r\n");
//         osMutexRelease(tcp_send_mutex);
//         return;
//     }
...
//         LOG_NET("tcp_send_str: json too long (%u), drop\r\n", (unsigned)json_len);
//         osMutexRelease(tcp_send_mutex);
//         return;
//     }
...
static void tcp_send_heartbeat(HeartBeatParam_t *p_param)
{
    ...
    //LOG_NET("TCP Sorted heartbeat JSON:\r\n%s\r\n", json_without_sign);
    uint8_t hash[32];
    hmac_sha256_init(&hmac, (const uint8_t *)HMAC_KEY, strlen(HMAC_KEY));
...
void tcp_send_state_response(StateRespParam_t *p_param)
{
    ...
    //LOG_NET("TCP Sorted state_response JSON:\r\n%s\r\n", json_without_sign);
...
void tcp_send_all_state_response(void)
{
    ...
    if(str)
    {
        ...
        if(msg.data) {
            ...
            if(xQueueSend(tcp_send_queue, &msg, 0) != pdPASS) {
                vPortFree(msg.data);          // 队列满，丢弃
                LOG_NET("TCP send queue full, drop all state_response");
            }
        }
        cJSON_PortFree(str);
        g_tcp_seq++;
...
static void tcp_parse_cmd(struct tcp_pcb *tpcb, char *buf)
{
    ...
    if(strcmp(type->valuestring, "task_order") == 0)
    {
        ...
        if(sign_src)
        {
            //LOG_NET("TCP Sorted task_order JSON:\r\n%s\r\n", sign_src);
        }
    }
    else if(strcmp(type->valuestring, "state_request") == 0)
    {
        ...
        {
            LOG_NET("state_request inj_id invalid!\r\n");
            goto exit;
        }
        ...
        //LOG_NET("TCP Sorted state_request JSON:\r\n%s\r\n", tcp_sign_buf_2048);
    ...
    else if(strcmp(type->valuestring, "task_empty") == 0)
    {
        ...
        //LOG_NET("TCP Sorted task_empty JSON:\r\n%s\r\n", tcp_sign_buf_2048);
        sign_src = tcp_sign_buf_2048;
    }
    else if(strcmp(type->valuestring, "stop_task") == 0)
    {
        ...
        snprintf(tcp_sign_buf_2048, sizeof(tcp_sign_buf_2048),
            ...
            task_id->valuestring,
            ts_cmd_str);
        //LOG_NET("TCP Sorted stop_task JSON:\r\n%s\r\n", tcp_sign_buf_2048);
        sign_src = tcp_sign_buf_2048;
    }
    else if(strcmp(type->valuestring, "heartbeat_ack") == 0)
    {
        ...
        LOG_NET("Server time: %s, Has pending task: %d\r\n", server_ts_str, pending_task);
        ...
        //LOG_NET("TCP Sorted heartbeat_ack JSON:\r\n%s\r\n", tcp_sign_buf_2048);
        sign_src = tcp_sign_buf_2048;
    }
    ...
    if(strcmp(type->valuestring, "state_request") == 0)
    {
        ...
        {
            LOG_NET("state_request inj_id out of range 1~%d\r\n", INJECTOR_CNT);
        }
    }
    if(strcmp(type->valuestring, "stop_task") == 0)
    {
        ...
        {
            g_force_stop_task = 1;
            LOG_NET("Valid stop_task cmd matched task_id:%s, trigger emergency stop\r\n", g_stop_target_task_id);
        }
        else
        {
            LOG_NET("stop_task task_id mismatch, current tcp task:%s, req stop:%s\r\n", g_tcp_task_bind.task_id, g_stop_target_task_id);
        }
    }
...
void tcp_send_task_request(TaskInfo_t *info_arr, uint16_t arr_len, uint8_t is_oiling)
{
    ...
    {
        LOG_NET("task info array is empty!\r\n");
        return;
    }
    ...
    // LOG_NET("TCP Sorted task_request JSON:\r\n%s\r\n", json_no_sign);
    ...
    if(str)
    {
        ...
        if(msg.data) {
            ...
            if(xQueueSend(tcp_send_queue, &msg, 0) != pdPASS) {
                vPortFree(msg.data);          // 队列满，丢弃
                LOG_NET("TCP send queue full, drop task_request");
            }
        }
        cJSON_PortFree(str);
        g_tcp_seq++;
...
void tcp_send_execute_result(ExecuteResultInfo_t *info_arr, uint16_t arr_len)
{
    ...
    if(info_arr == NULL || arr_len == 0)
    {
        LOG_NET("execute result array is empty!\r\n");
        return;
    }
    ...
    if(str) 
    {
        ...
        if(msg.data) {
            ...
            if(xQueueSend(tcp_send_queue, &msg, 0) != pdPASS) {
                vPortFree(msg.data);          // 队列满，丢弃
                LOG_NET("TCP send queue full, drop execute_result");
            }
        }
...
/**
 * @brief TCP任务内部统一失败上报封装
 */
static void tcp_report_fail(const char *task_id, uint8_t inj_id, uint8_t err_code, const char *err_msg)
{
    ExecuteResultInfo_t exec_info; // 局部变量，栈内分配，无并发风险
    memset(&exec_info, 0, sizeof(exec_info));

    // 安全拷贝task_id
    if (task_id != NULL)
    {
        strncpy(exec_info.task_id, task_id, sizeof(exec_info.task_id) - 1);
    }
    else
    {
    ...
    exec_info.inj_id = inj_id;
    exec_info.actual_volume = 0;
    exec_info.valve_act_tm = 0;
    exec_info.valve_total_tm = 0;

    // 读取TimeSyncTask定时刷新的全局温湿度缓存（放大10倍整型）
    exec_info.valve_temp = g_sht40_temp_cache;
    exec_info.valve_hum  = g_sht40_hum_cache;

    exec_info.execute_result = 3;
    exec_info.error_code = err_code;
    strncpy(exec_info.error_msg, err_msg, sizeof(exec_info.error_msg) - 1);
...
void TCP_task_processing_task(void *argument)
{
    ...
    while(1)
    {
        if(xQueueReceive(xTcpTaskQueue, &recv_item, portMAX_DELAY) == pdPASS)
        {
            ...
            {
                LOG_NET("System not ready mode, discard tcp task: inj= %u; vol= %lu; inj_ts= %lu; task_id= %s\r\n",
                    recv_item.inj_id, (unsigned long)recv_item.inj_v, (unsigned long)recv_item.inj_ts, recv_item.task_id);
//-------------------------------失败上报------------------------------------------------------------------------------------------------    
                tcp_report_fail(recv_item.task_id, recv_item.inj_id, 6, "The system is not ready !");
//-------------------------------------------------------------------------------------------------------------------------------
                continue;
            }
            ...
            {
                LOG_NET("System busy, discard tcp task: inj= %u; vol= %lu; inj_ts= %lu; task_id= %s\r\n",
                    recv_item.inj_id, (unsigned long)recv_item.inj_v, (unsigned long)recv_item.inj_ts, recv_item.task_id);
//-------------------------------失败上报------------------------------------------------------------------------------------------------    
                tcp_report_fail(recv_item.task_id, recv_item.inj_id, 7, "The system is busy !");
//-------------------------------------------------------------------------------------------------------------------------------
                continue;
            }
...
//             {
//                 LOG_NET("main_sys_status.p_model.status = %d\r\n", main_sys_status.p_model.status);
//                 LOG_NET("main_sys_status.q_model.motor_status = %d\r\n", main_sys_status.q_model.motor_status);
//                 LOG_NET("main_sys_status.q_model.sen_status = %d\r\n", main_sys_status.q_model.sen_status);
// //-------------------------------失败上报------------------------------------------------------------------------------------------------    
...
//             }
                ...
                if(inj_id >= 1 && inj_id <= injector_count)
                {
                    ...
                    if(new_sec >= 5 && new_sec <= 3456000)
                    {
                        if(injector_config.injector[tag].interval != new_sec)
                        {
                            ...
                            LOG_NET("TCP update injector%d auto interval to %llu s\r\n", inj_id, new_sec);
                            config_changed = 1;
                        }
                    }
                    else
                    {
                        LOG_NET("TCP invalid interval inj%d, ts_sec:%llu\r\n", inj_id, new_sec);
                    }
                }
                else
                {
                    LOG_NET("TCP modify interval invalid inj_id:%d\r\n", inj_id);
                }
            }
                ...
                if(inj_id >= 1 && inj_id <= injector_count)
                {
                    ...
                    if(new_volume >= VOLUME_MIN && new_volume <= VOLUME_MAX)
                    {
                        if(injector_config.injector[tag].volume != new_volume)
                        {
                            ...
                            LOG_NET("TCP update injector%u config volume to %lu mL\r\n", inj_id, (unsigned long)new_volume);
                            config_changed = 1;
                        }
                    }
                    else
                    {
                        LOG_NET("TCP invalid volume inj%u, volume:%lu\r\n", inj_id, (unsigned long)new_volume);
                    }
                }
                else
                {
                   LOG_NET("TCP modify volume invalid inj_id:%u\r\n", inj_id);
                }
            }
            ...
            {
                LOG_NET("task_object_list[3].task_status %d\r\n",task_object_list[idx].task_status);
                ...
                LOG_NET("TCP task write slot3 ok: inj= %u; vol= %lu; inj_ts= %lu; task_id= %s\r\n",
                    recv_item.inj_id, (unsigned long)recv_item.inj_v, (unsigned long)recv_item.inj_ts, recv_item.task_id);
                // 仅任务成功落地时，更新全局任务ID
                ...
                g_tcp_task_bind.task_id[sizeof(g_tcp_task_bind.task_id) - 1] = '\0';
                g_tcp_task_bind.bind_inj_id = recv_item.inj_id;
            }
            else
            {
                // 槽位繁忙、任务丢弃，不更新 g_task_id
                LOG_NET("slot3 busy,discard tcp task: inj= %u; vol= %lu; inj_ts= %lu; task_id= %s\r\n",
                    recv_item.inj_id, (unsigned long)recv_item.inj_v, (unsigned long)recv_item.inj_ts, recv_item.task_id);
//-------------------------------失败上报------------------------------------------------------------------------------------------------    
                tcp_report_fail(recv_item.task_id, recv_item.inj_id, 9, "The task list is full !");
//-------------------------------------------------------------------------------------------------------------------------------
            }
        }
    }
...
void timer_req_send_task(void *arg)
{
    ...
    for(;;)
    {
        if(xQueueReceive(xTimerReqQueue, &item, pdMS_TO_TICKS(50)) == pdPASS)
        {
            ...
            {
                LOG_NET("TCP offline, drop timer notify inj:%d\r\n", item.inj_id);
                continue;
            }
            ...
            if(g_task_request_cnt < TASK_DATA_MAX)
            {
                ...
                g_task_request_cnt++;
                LOG_NET("Cache inj task: %d, total:%d\r\n", item.inj_id, g_task_request_cnt);
            }
            else
            {
                LOG_NET("Task buffer full, drop inj:%d\r\n", item.inj_id);
                continue;
            }
           // 聚合窗口：500ms 收集短时内所有后续任务
            while(xQueueReceive(xTimerReqQueue, &item, pdMS_TO_TICKS(500)) == pdPASS)
            {
                ...
                {
                    LOG_NET("TCP offline, drop inj:%d\r\n", item.inj_id);
                    break;
                }

                if(main_sys_status.p_model.status != MOTOR_STATUS_READY || main_sys_status.q_model.motor_status != MOTOR_STATUS_READY || main_sys_status.q_model.sen_status != SENSOR_STATUS_READY)//P、Q、传感器未就绪
                {
                    LOG_NET("main_sys_status.P/Q/S not ready\r\n");
                    //continue;
                }
            ...
            if(g_task_request_cnt > 0)
            {
                ...
                // 批量发送：传数组首地址 + 有效数量
                //LOG_NET("Batch send %d inj tasks\r\n", g_task_request_cnt);
...
void vRebootTask(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        if(dev_busy == 1)
        {
            ...
            LOG_NET("Reject reboot: device busy, will reboot after oil filling finish, reason:%s\r\n", cmd.reason);
            // 循环阻塞等待，直到注油全部完成
            while(1)
            {
                taskENTER_CRITICAL();
        ...
        LOG_NET("Accept reboot: delay = %lus, reason = %s\r\n", cmd.delay_sec, cmd.reason);
        ...
        while(1)
        {
            ...
            if(dev_busy == 0)
                break;
            LOG_NET("After flash write, device busy again, re-wait");
            vTaskDelay(pdMS_TO_TICKS(100));
        }
...
void test_oil_filling_task(void *arg)
{
    ...
    for(;;)
    {
        ...
        LOG_NET("Start oil filling, inj_id:%d, target total vol:%d mL\r\n",
        ...
        while(fill_cur < fill_total)
        {
            ...
            if(g_force_stop_task == 1)
            {
                ...
                LOG_NET("Remote stop task triggered, break filling loop\r\n");
                break;
            }
            fill_cur++; // 1mL
            //记录当前mL开始时间戳
            ...
            // 单条进度上报
            tcp_send_execute_result(&exec_info, 1U);
            LOG_NET("Filling progress: current %d mL / total %d mL\r\n", fill_cur, fill_total);
        }
        memset(&exec_info, 0, sizeof(ExecuteResultInfo_t));
        strncpy(exec_info.task_id, g_tcp_task_bind.task_id, sizeof(exec_info.task_id)-1);
        exec_info.task_id[sizeof(exec_info.task_id)-1] = '\0';
        ...
        if(fill_err_flag)
        {
            LOG_NET("Oil filling FAIL, inj_id:%d, err_code:%d, msg:%s\r\n",
                task_object_list[slot_idx].inject_id, fill_err_code, fill_err_str);
        }
        else
        {
            LOG_NET("Oil filling finished, inj_id:%d, actual total vol:%u mL, total run time:%lu ms\r\n",
