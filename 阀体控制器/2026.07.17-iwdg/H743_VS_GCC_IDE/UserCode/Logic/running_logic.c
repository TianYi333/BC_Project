...
void start_injector_task(void) {
  injector_task_handler =
      osThreadNew(injector_task, NULL, &injector_task_attributes);
}
...
//-------------------------------温湿度采集（扩大10倍存整型）------------------------------------------------------------------------------------------------
  ...
  uint16_t valve_temp_val, valve_hum_val;
  ...
  if(temp_raw >= -40.0 && temp_raw <= 125.0 && hum_raw >= 0.0 && hum_raw <= 100.0)
  {
      ...
      if(temp_x10 > 2000) temp_x10 = 2000;
      if(hum_x10  < 0)   hum_x10  = 0;
      if(hum_x10  > 1000) hum_x10 = 1000;

      valve_temp_val = (uint16_t)temp_x10;
      valve_hum_val  = (uint16_t)hum_x10;
  }
  else
  {
      // 数值异常（未接传感器/通信失败），使用默认值
      valve_temp_val = 450;
      valve_hum_val  = 380;
  }
//-------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    exec_info.error_code = 1;//错误码
    strncpy(exec_info.error_msg, "Q_MODEL_RUN_CLOSE_FAILURE !", sizeof(exec_info.error_msg)-1);
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    exec_info.valve_hum = valve_hum_val;
    exec_info.execute_result = 3;//错误
    exec_info.error_code = 2;//错误码
    strncpy(exec_info.error_msg, "P_MODEL_RUN_POINT_FAILURE !", sizeof(exec_info.error_msg)-1);
    exec_info.error_msg[sizeof(exec_info.error_msg)-1] = '\0';
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
      ...
      exec_info.valve_hum = valve_hum_val;
      exec_info.execute_result = 3;//错误
      exec_info.error_code = 3;//错误码
      strncpy(exec_info.error_msg, "Q_MODEL_RUN_OPEN_FAILURE !", sizeof(exec_info.error_msg)-1);
      exec_info.error_msg[sizeof(exec_info.error_msg)-1] = '\0';
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
      ...
      exec_info.valve_hum = valve_hum_val;
      exec_info.execute_result = 3;//错误
      exec_info.error_code = 4;//错误码
      strncpy(exec_info.error_msg, "WATING_MET_SEN_FAILURE !", sizeof(exec_info.error_msg)-1);
      exec_info.error_msg[sizeof(exec_info.error_msg)-1] = '\0';
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    exec_info.valve_hum = valve_hum_val;
    exec_info.execute_result = 3;//错误
    exec_info.error_code = 5;//错误码
    strncpy(exec_info.error_msg, "Q_MODEL_RETURN_CLOSE_FAILURE !", sizeof(exec_info.error_msg)-1);
    exec_info.error_msg[sizeof(exec_info.error_msg)-1] = '\0';
...
//---------------------------------成功上报----------------------------------------------------------------------------------------------
  ...
  exec_info.actual_volume = fill_cur;      // 实际总出油量
  exec_info.valve_act_tm = action_time;
  exec_info.valve_total_tm = total_cost_ms;

  exec_info.valve_temp = valve_temp_val;
  exec_info.valve_hum = valve_hum_val;
  exec_info.execute_result = 2;//成功
  exec_info.error_code = 0;
