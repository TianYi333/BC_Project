...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    // 记录当前出油口故障
    record_injector_err(injector_id, Q_MODEL_RESET_FAILURE, "Q_MODEL_RESET_FAILURE");
//-------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------失败上报------------------------------------------------------------------------------------------------
    ...
    LOG("Oil filling FAIL, inj_id:%d, err_code:%d, msg:%s\r\n",injector_id, exec_info.error_code, exec_info.error_msg);
    // 记录当前出油口故障
    record_injector_err(injector_id, P_MODEL_RESET_FAILURE, "P_MODEL_RESET_FAILURE");
//-------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------成功上报----------------------------------------------------------------------------------------------
    ...
    exec_info.actual_volume = fill_cur;      // 实际总出油量
    exec_info.valve_act_tm = (uint16_t)action_time;
    exec_info.valve_total_tm = (uint16_t)total_cost_ms;
