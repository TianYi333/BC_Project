...
void PrintTaskStackAndCount(void)
{
    ...
    LOG("1 ,StartDefaultTask          min stack left: %u bytes", uxTaskGetStackHighWaterMark(defaultTaskHandle));
    LOG("2 ,OLED_Task_Entry           min stack left: %u bytes", uxTaskGetStackHighWaterMark(g_oledTaskHandle));
    LOG("3 ,OLED_Msg_Task_Entry       min stack left: %u bytes", uxTaskGetStackHighWaterMark(OLED_Msg_TaskkHandle));
    LOG("4 ,buzz_task                 min stack left: %u bytes", uxTaskGetStackHighWaterMark(buzz_task_handler));
    LOG("5 ,syncif_task               min stack left: %u bytes", uxTaskGetStackHighWaterMark(syncif_task_handler));
    LOG("6 ,syncif_task_cycle         min stack left: %u bytes", uxTaskGetStackHighWaterMark(syncif_task_handler_cycle));
    LOG("7 ,mbif_task                 min stack left: %u bytes", uxTaskGetStackHighWaterMark(mbif_task_handler));
    LOG("8 ,injector_task             min stack left: %u bytes", uxTaskGetStackHighWaterMark(injector_task_handler));
    LOG("9 ,ETH_InitTask              min stack left: %u bytes", uxTaskGetStackHighWaterMark(ETH_InitTaskHandle));
    LOG("10,udp_discover_task         min stack left: %u bytes", uxTaskGetStackHighWaterMark(udpTaskHandle));
    LOG("11,tcp_client_task           min stack left: %u bytes", uxTaskGetStackHighWaterMark(tcpConfigTaskHandle));
    LOG("12,TCP_task_processing_task  min stack left: %u bytes", uxTaskGetStackHighWaterMark(TCP_task_processingTaskHandle));
    LOG("13,TimerReqSendTask          min stack left: %u bytes", uxTaskGetStackHighWaterMark(TimerReqSendTaskHandle));
    LOG("14,TimeSyncTask              min stack left: %u bytes", uxTaskGetStackHighWaterMark(TimeSyncTaskHandle));
